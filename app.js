/* ═══════════════════════════════════
   COLORS — نفس قيم :root في CSS
   عايز تغير؟ غيّر هنا وفي :root
═══════════════════════════════════ */
const COLORS = {
  'accent': '#E8E0D0',
  'accent-gold': '#C9A84C',
  ...
  'purple-soft': '#9B7ACC',
  'success': '#27AE60',
  'success-ghost': '#F0FFF4',
  'success-glow': '#EAF5EE',
  'success-muted': '#C8E6C9',
  'success-pale': '#E8F5E9',
  'success-soft': '#7DBFA0',
  'text-body': '#555',
  'text-dark': '#1A1A1A',
  ...
  'warning-alt': '#E0A800',
  'warning-bg': '#FFF3CD',
  'warning-cream': '#F0EBE0',
  'warning-dark': '#E65100',
  'warning-faint': '#FFFBF0',
  'warning-ghost': '#FFFDE7',
...
async function backupAll(){
  ...
  try{
    ...
    wb.created=new Date();
    const G='${COLORS['primary']}',B='${COLORS['accent']}';
    ...
    msg.style.color='${COLORS['primary-btn']}';
  }catch(e){
    setSav('❌ خطأ: '+e.message,'er');
    msg.style.color='${COLORS['danger']}';
  }
  btn.disabled=false;
}
...
async function loadDashboard(){
  try{
    ...
    balEl.className='d-kpi-val net'+(bal<0?' exp':'');

    // Hero
    const heroEl=document.getElementById('dHeroVal');
    heroEl.textContent=(bal>=0?'+':'')+fn(bal)+' ج';
    heroEl.style.color=bal<0?'${COLORS['danger-soft']}':'${COLORS['success-soft']}';
    document.getElementById('dHeroSub').textContent='إجمالي الوارد − المصاريف المباشرة − العهد المفتوحة';

    // Progress bar (مصروف / وارد)
    const heroPct=totalInc>0?Math.min(100,Math.round((totalExp/totalInc)*100)):0;
    const heroBar=document.getElementById('dHeroBar');
    if(heroBar){
      heroBar.style.width=heroPct+'%';
      heroBar.style.background=heroPct>90?'linear-gradient(90deg,${COLORS['danger-soft']},${COLORS['accent-gold']})':heroPct>70?'linear-gradient(90deg,${COLORS['accent-gold']},${COLORS['success-soft']})':'linear-gradient(90deg,${COLORS['success-soft']},${COLORS['info-sky']})';
    }

    // Projects
    const container=document.getElementById('dProjCards');
    const countEl=document.getElementById('dProjCount');
    if(!allProjects.length){container.innerHTML='<div class="empty-state">لا توجد مشاريع</div>';return;}
    if(countEl)countEl.textContent=allProjects.length+' مشروع';
    initDashFilter();
    ...
    const projColors=['${COLORS['success-soft']}','${COLORS['info-sky']}','${COLORS['accent-gold']}','${COLORS['purple-soft']}','${COLORS['danger-peach']}','${COLORS['danger-blush']}','${COLORS['info-soft']}','${COLORS['danger-warm']}'];
    ...
    container.innerHTML=sortedProjects.map((p,idx)=>{
      ...
      const fillColor=pct>90?'${COLORS['danger-soft']}':pct>70?'${COLORS['accent-gold']}':color;
      const badgeTxt=pB>0&&pI>0?'مستقر ✦':pB<0?'عجز ⚠':pI===0?'بدون وارد':'تحت ضغط';
      return `<div class="d-pcard" onclick="showScreen('proj');setTimeout(()=>{document.getElementById('ps').value='${p.id}';sw('${p.id}');},100)" style="animation-delay:${idx*0.06}s">
        <div class="d-pcard-head">
          <div class="d-pcard-name" style="color:${color}">${p.name}</div>
          <span class="d-pcard-badge ${balCls}">${badgeTxt}</span>
        </div>
        <div class="d-pcard-stats">
          <div class="d-pcard-stat">
            <div class="d-pcard-stat-lbl">وارد</div>
            <div class="d-pcard-stat-val inc">+${fn(pI)}</div>
          </div>
          <div class="d-pcard-stat">
            <div class="d-pcard-stat-lbl">مصروف</div>
            <div class="d-pcard-stat-val exp">-${fn(pE)}</div>
          </div>
        </div>
        <div class="d-pcard-progress">
          <div class="d-pcard-progress-info">
            <span>نسبة الصرف</span>
            <span>${pct}%</span>
          </div>
          <div class="d-pcard-progress-bar">
            <div class="d-pcard-progress-fill" style="width:${pct}%;background:${fillColor}"></div>
          </div>
        </div>
        <div class="d-pcard-footer">
          <div class="d-pcard-bal ${balCls}">${pB>=0?'+':''}${fn(pB)} ج</div>
          <div class="d-pcard-meta">${s.count} قيد · ${s.cats.length} بند</div>
        </div>
      </div>`;
    }).join('');
...
async function saveProjectEdit(){
  ...
  if(!name){msg.style.color='${COLORS['danger']}';msg.textContent='❌ الاسم مطلوب';return;}
  msg.style.color='${COLORS['warning-text']}';msg.textContent='⏳ جاري الحفظ...';
  try{
    ...
    msg.style.color='${COLORS['primary-btn']}';msg.textContent='✅ تم الحفظ';
    setSav('✅ تم تعديل المشروع','ok');
    setTimeout(()=>document.getElementById('editProjModal')?.remove(),700);
  }catch(e){msg.style.color='${COLORS['danger']}';msg.textContent='❌ خطأ: '+e.message;}
}
...
function rp(){
  ...
  (document.getElementById('kp')||{}).innerHTML='<div class="kc"><div class="kl">الوارد</div><div class="kv" style="color:${COLORS['info']}">'+fn(inc)+'</div></div><div class="kc"><div class="kl">المصروف</div><div class="kv" style="color:${COLORS['danger']}">'+fn(exp)+'</div></div><div class="kc"><div class="kl">'+(bal<0?'⚠ عجز':'✅ الرصيد')+'</div><div class="kv" style="color:'+(bal<0?'${COLORS['danger']}':'${COLORS['primary-btn']}')+'">'+fn(bal)+'</div></div>';
...
function re(){
  ...
  if(cTab==='m'){
    ...
    el.innerHTML=mqs.map((m,idx)=>{
      ...
      const addBtn=canEdit?`<button onclick="event.stopPropagation();mqAddByIdx(${idx})" class="is3">+ قيد</button>`:'';
      const printBtn=`<button onclick="event.stopPropagation();mqPrintReport(${idx})" class="is4">🖨️ تقرير</button>`;
      const rows=m.rows.sort((a,b)=>pdt(b.entry_date)-pdt(a.entry_date)).map(e=>{
        ...
        const etBg={'payment':'${COLORS['success-pale']}','work':'${COLORS['info-bg']}','material':'${COLORS['warning-pale']}'};
        const etC={'payment':'${COLORS['primary-btn']}','work':'${COLORS['info']}','material':'${COLORS['warning-dark']}'};
        ...
        return `<div class="rw${canEdit?' clk':''}" onclick="oe('${e.id}')"><div class="ri"><div class="rd">${tag} ${e.description||'—'} <span class="nb">#${e.seq||e.entry_no||'?'}</span></div><div class="rm">${e.entry_date||'—'} · ${e.category||'—'}</div></div><div class="flex-center-gap"><div class="ra">${fn(e.amount)} ج</div>${del}</div></div>`;
      }).join('');
      const kpis=hasTypes?`<div class="is5"><div class="kpi-inc"><div class="lbl-sm">💰 دفعات</div><div class="kpi-val-inc">${fn(m.pay)}</div></div><div class="kpi-work"><div class="lbl-sm">🔨 أعمال</div><div class="kpi-val-work">${fn(m.work)}</div></div><div class="kpi-mat"><div class="lbl-sm">🔩 مصنعيات</div><div class="kpi-val-mat">${fn(m.mat)}</div></div><div style="background:${rem>=0?'${COLORS['success-ghost']}':'${COLORS['danger-ghost']}'};border-radius:8px;padding:8px;text-align:center"><div class="lbl-sm">${rem>=0?'الباقي معاه':'مستحق عليك'}</div><div style="font-weight:900;color:${rem>=0?'${COLORS['primary']}':'${COLORS['danger']}'};font-size:13px">${fn(Math.abs(rem))}</div></div></div>`:`<div class="is6"><span style="color:${COLORS['text-soft']};font-size:12px">إجمالي المسحوب</span><span style="font-weight:700;color:${COLORS['primary']}">${fn(m.pay+m.work+m.mat+m.other)} ج</span></div>`;
      return `<div class="is7"><div class="is8" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'block':'none'"><div class="is9"><span class="is10">👷 ${m.n}</span><div style="display:flex;gap:6px;align-items:center">${printBtn}${addBtn}<span class="is11">${m.rows.length} قيد ▼</span></div></div></div><div style="padding:14px 16px">${kpis}<div>${rows}</div></div></div>`;
    }).join('');
    return;
  }
  ...
  if(hasMqTypes){
    html+=`<div style="display:flex;gap:6px;margin-bottom:10px">
      <button onclick="setCatView('list',this)" class="is12" id="cvList">📋 القيود</button>
      <button onclick="setCatView('mq',this)" class="is13" id="cvMq">👷 المقاولين</button>
  ...
  html+=es.map(e=>{
    ...
    const etBg=e.entry_type==='payment'?'${COLORS['success-pale']}':e.entry_type==='work'?'${COLORS['info-bg']}':e.entry_type==='material'?'${COLORS['warning-pale']}':'';
    const etC=e.entry_type==='payment'?'${COLORS['primary-btn']}':e.entry_type==='work'?'${COLORS['info']}':e.entry_type==='material'?'${COLORS['warning-dark']}':'';
    const et=e.entry_type&&e.contractor?'<span class="is14">'+(etypeLbl[e.entry_type]||'')+'</span>':'';
  ...
  if(hasMqTypes){
    ...
    html+=Object.entries(mqMap).map(([name,d])=>{
      ...
      const rows=d.rows.map(e=>{
        ...
        const etC=e.entry_type==='payment'?'${COLORS['primary-btn']}':e.entry_type==='work'?'${COLORS['info']}':'${COLORS['warning-dark']}';
        const etBg=e.entry_type==='payment'?'${COLORS['success-pale']}':e.entry_type==='work'?'${COLORS['info-bg']}':'${COLORS['warning-pale']}';
        return '<div class="is15"><span class="is16">'+et+'</span><span style="flex:1;color:${COLORS['text-mid']}">'+(e.description||'—')+'</span><span style="font-weight:700;color:${COLORS['text-body']}">'+fn(e.amount)+' ج</span></div>';
      }).join('');
      return '<div class="is17"><div class="is18"><span class="is19">👷 '+name+'</span></div><div style="padding:12px 14px"><div class="is20"><div class="kpi-inc"><div class="lbl-sm">💰 دفعات</div><div class="kpi-val-inc">'+fn(d.pay)+'</div></div><div class="kpi-work"><div class="lbl-sm">🔨 أعمال</div><div class="kpi-val-work">'+fn(d.work)+'</div></div><div class="kpi-mat"><div class="lbl-sm">🔩 مصنعيات</div><div class="kpi-val-mat">'+fn(d.mat)+'</div></div></div><div class="is21"><span style="font-size:12px;color:${COLORS['text-muted']}">الباقي معاه</span><span class="is22">'+fn(rem)+' ج</span></div>'+rows+'</div></div>';
    }).join('');
    html+='</div>';
  }
  el.innerHTML=html;
...
async function loadAdvList(){
  ...
  try{
    ...
    al.innerHTML=advances.map(a=>{
      ...
      const ownerBadge=ownerUser&&uRole==='admin'?'<span class="is23">🔗 '+ownerUser.name+'</span>':'';
      const remColor=rem<0?'${COLORS['danger']}':rem===0?'${COLORS['primary-btn']}':'${COLORS['warning-text']}';
      return '<div class="adv-card" onclick="openAdv(\''+a.id+'\')"><div class="adv-card-h"><div class="adv-name">👤 '+a.person_name+ownerBadge+'</div><span class="adv-status '+(a.status==='open'?'open':'closed')+'">'+(a.status==='open'?'⏳ مفتوحة':'✅ مغلقة')+'</span></div>'+(a.notes?'<div class="is24">'+a.notes+'</div>':'')+'<div class="adv-nums"><div class="adv-num"><div class="adv-num-l">العهدة</div><div class="adv-num-v" style="color:${COLORS['info']}">'+fn(totalGiven)+'</div></div><div class="adv-num"><div class="adv-num-l">صرف</div><div class="adv-num-v" style="color:${COLORS['danger']}">'+fn(spent)+'</div></div><div class="adv-num"><div class="adv-num-l">الباقي</div><div class="adv-num-v" style="color:'+remColor+'">'+fn(rem)+'</div></div></div><div class="is25"><div class="is26"></div></div></div>';
    }).join('');
...
async function openAdv(id){
  ...
  btn.style.background=isOpen?'${COLORS['primary-btn']}':'${COLORS['warning-text']}';
  btn.style.color='#fff';
  ...
  if(viewerCanAdd){
    ...
    dateField.style.background='${COLORS['bg-light']}';
    dateField.style.color='${COLORS['text-hint']}';
...
async function loadAdvDetail(){
  ...
  if(uRole==='viewer'){
    ...
    if(df){
      ...
      df.style.background='${COLORS['bg-light']}';
      df.style.color='${COLORS['text-hint']}';
    }
  }
  ...
  try{
    ...
    var clr=rem>0?'${COLORS['warning-text']}':'${COLORS['primary-btn']}';
    kp.innerHTML=`<div class='kc'><div class='kl'>&#x625;&#x62c;&#x645;&#x627;&#x644;&#x64a; &#x627;&#x644;&#x62f;&#x641;&#x639;&#x627;&#x62a;</div><div class='kv' style='color:${COLORS['info']}'>${fn(totalGiven)} &#x62c;</div></div><div class='kc'><div class='kl'>&#x635;&#x631;&#x641;</div><div class='kv' style='color:${COLORS['danger']}'>${fn(spent)} &#x62c;</div></div><div class='kc'><div class='kl'>&#x627;&#x644;&#x628;&#x627;&#x642;&#x64a;</div><div class='kv' style='color:${clr}'>${fn(rem)} &#x62c;</div></div>${pendingSpent>0?`<div class='kc'><div class='kl'>&#x2935;&#xfe0f; &#x641;&#x64a; &#x627;&#x644;&#x627;&#x646;&#x62a;&#x638;&#x627;&#x631;</div><div class='kv' style='color:${COLORS['warning-text']}'>${fn(pendingSpent)} &#x62c;</div></div>`:''}`;
    if(installs.length===0){
      il.innerHTML=`<div class='emp'>&#x644;&#x627; &#x62a;&#x648;&#x62c;&#x62f; &#x62f;&#x641;&#x639;&#x627;&#x62a; &#x628;&#x639;&#x62f;</div>`;
    }else{
      il.innerHTML=installs.map(ins=>{
        var db=uRole!=='viewer'?`<button class='db' onclick='delInstall("${ins.id}")'>&${COLORS['dark-emerald']};</button>`:'';
    ...
    if(advEntries.length===0&&pendingAdvEntries.length===0){
      ...
      const approvedHtml=advEntries.map(e2=>{
        ...
        var db2=canEditAdv?`<button class='db' onclick='editAdvEntry("${e2.id}")' style='color:${COLORS['primary']}'>&#9998;</button><button class='db' onclick='delAdvEntry("${e2.id}")'>&${COLORS['dark-emerald']};</button>`:'';
      ...
      const pendingHtml=pendingAdvEntries.map(e2=>{
        var pName=projMap[e2.project_id]||'&mdash;';
        return `<div class='rw' style='opacity:.75;border:1px dashed ${COLORS['accent-gold']};background:${COLORS['warning-ghost']}'><div class='ri'><div class='rd'>&#x23f3; ${e2.description||'&mdash;'} <span style='font-size:10px;color:${COLORS['warning-text']};background:${COLORS['warning-bg']};padding:1px 6px;border-radius:8px'>في الانتظار</span></div><div class='rm'>${pName} &middot; ${e2.category||'&mdash;'} &middot; ${cleanDate(e2.entry_date)}</div></div><div style='display:flex;align-items:center'><div class='ra neg' style='color:${COLORS['warning-text']}'>${fn(e2.amount)} &#x62c;</div></div></div>`;
      }).join('');
...
async function loadAdminPanel(){
  try{
    ...
    ul.innerHTML=allP.map(u=>{
      ...
      const isMe=u.id===uid;
      return '<div class="ucard"><div class="ucard-h"><div><div class="uname">'+u.name+'</div></div><div style="display:flex;gap:6px;align-items:center">'+(!isMe?'<button class="del-u" onclick="delUser(\''+u.id+'\',\''+u.name+'\')">حذف</button>':'')+'<select class="rsel" onchange="chRole(\''+u.id+'\',this.value)"'+(isMe?' disabled':'')+' ><option value="admin" '+(u.role==='admin'?'selected':'')+'>👑 أدمن</option><option value="editor" '+(u.role==='editor'?'selected':'')+'>✏️ محاسب</option><option value="viewer" '+(u.role==='viewer'?'selected':'')+'>👁 مشاهد</option></select></div></div><div class="is27">المشاريع:</div><div class="pchips">'+chips+'</div></div>';
    }).join('');
...
function renderCatOpts(q){
  ...
  if(!html)html='<div class="is29">لا توجد بنود بعد<br><small>اكتب بند جديد وسيُضاف تلقائياً</small></div>';
  list.innerHTML=html;
}
...
function checkPwdStrength(v){
  ...
  const pct=score*20;
  const colors=['${COLORS['danger-soft']}','${COLORS['warning']}','${COLORS['warning']}','${COLORS['success-soft']}','${COLORS['success']}'];
  bar.style.width=pct+'%';
  bar.style.background=colors[Math.min(score-1,4)]||'${COLORS['danger-soft']}';
}
  ...
  el.innerHTML=`
    <div class="filter-result">
      ...
      <div style="display:flex;gap:8px;margin-bottom:10px">
        ...
        <span class="is31">${filtered.length} قيد</span>
      </div>
...
const _PDF_CSS=`
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:Arial,sans-serif;direction:rtl;background:${COLORS['bg-gray']};color:${COLORS['text-dark']}}
  .page{background:#fff;max-width:960px;margin:0 auto;padding:36px 40px;min-height:100vh}
  /* ── HEADER ── */
  .hdr{display:flex;justify-content:space-between;align-items:center;padding-bottom:16px;border-bottom:4px solid ${COLORS['primary']};margin-bottom:24px}
  .hdr-left h1{font-size:24px;font-weight:900;color:${COLORS['primary']};margin-bottom:3px}
  .hdr-left .sub{font-size:12px;color:${COLORS['text-soft']};line-height:1.6}
  ...
  .hdr-badge.owner{background:${COLORS['success-glow']};color:${COLORS['primary-btn']};border:1px solid ${COLORS['success-muted']}}
  .hdr-badge.acct{background:${COLORS['info-bg']};color:${COLORS['info']};border:1px solid ${COLORS['info-muted']}}
  ...
  .kpi-inc{background:${COLORS['success-glow']};border-color:${COLORS['success-muted']};color:${COLORS['primary-btn']}}
  .kpi-exp{background:${COLORS['danger-pale']};border-color:${COLORS['danger-tint']};color:${COLORS['danger']}}
  .kpi-adv{background:${COLORS['info-bg']};border-color:${COLORS['info-muted']};color:${COLORS['info']}}
  .kpi-net-pos{background:${COLORS['success-glow']};border-color:${COLORS['success-muted']};color:${COLORS['primary-btn']}}
  .kpi-net-neg{background:${COLORS['danger-pale']};border-color:${COLORS['danger-tint']};color:${COLORS['danger']}}
  .kpi-neutral{background:${COLORS['bg-pure']};border-color:${COLORS['border-light']};color:${COLORS['text-body']}}
  /* ── SECTION TITLE ── */
  .sec-ttl{font-size:13px;font-weight:800;color:${COLORS['primary']};padding:10px 0;border-bottom:2px solid ${COLORS['border-light']};margin-bottom:0;display:flex;align-items:center;gap:6px}
  /* ── TABLE ── */
  table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:24px}
  thead tr{background:${COLORS['primary']}}
  th{color:${COLORS['accent']};padding:10px 8px;text-align:right;font-size:10px;font-weight:700;letter-spacing:.3px}
  th:last-child{text-align:center}
  td{padding:8px 8px;border-bottom:1px solid ${COLORS['bg-light']};vertical-align:middle}
  tr:nth-child(even) td{background:${COLORS['bg-linen']}}
  tr:last-child td{border-bottom:none}
  tfoot tr{background:${COLORS['bg-latte']}}
  tfoot td{padding:9px 8px;font-weight:800;border-top:2px solid ${COLORS['primary']}}
  /* ── BADGES ── */
  .b{display:inline-block;padding:2px 8px;border-radius:10px;font-size:9px;font-weight:700}
  .b-i{background:${COLORS['success-glow']};color:${COLORS['primary-btn']}}.b-e{background:${COLORS['danger-pale']};color:${COLORS['danger']}}
  .b-pay{background:${COLORS['success-glow']};color:${COLORS['primary-btn']}}.b-work{background:${COLORS['info-bg']};color:${COLORS['info']}}.b-mat{background:${COLORS['warning-pale']};color:${COLORS['warning-dark']}}
  /* ── AMOUNTS ── */
  .amt{white-space:nowrap;font-weight:700}
  .pos{color:${COLORS['primary-btn']}}.neg{color:${COLORS['danger']}}
  /* ── WATERMARK ── */
  ...
  /* ── CHART ── */
  .chart-wrap{width:100%;border-radius:10px;overflow:hidden;margin-bottom:20px;border:1px solid ${COLORS['border-faint']}}
  .chart-wrap img{width:100%;max-height:240px;object-fit:contain;display:block}
  /* ── FOOTER ── */
  .ftr{margin-top:28px;padding-top:16px;border-top:2px solid ${COLORS['border-ghost']};display:flex;justify-content:space-between;align-items:center;gap:16px}
  .ftr-logo{height:36px;opacity:.4;flex-shrink:0}
  .ftr-mid{text-align:center;flex:1}
  .ftr-company{font-size:12px;font-weight:800;color:${COLORS['primary']};margin-bottom:4px}
  .ftr-owner{font-size:10px;color:${COLORS['primary-btn']};font-weight:600;margin-bottom:2px}
  .ftr-acct{font-size:10px;color:${COLORS['info']};font-weight:600;background:${COLORS['info-bg']};display:inline-block;padding:2px 10px;border-radius:20px;margin-bottom:3px}
  .ftr-date{font-size:9px;color:${COLORS['text-faint']};margin-top:2px}
  .ftr-conf{font-size:9px;color:${COLORS['border-mid']};text-align:left;line-height:1.5;flex-shrink:0}
...
function _pdfFooter(){
  ...
  return `<div class="ftr">
    <img src="${LOGO_B64}" class="ftr-logo">
    <div class="ftr-mid">
...
// Excel helper — applies consistent styling to a worksheet
function _xlStyle(ws,hasHeader,hasTotals){
  const numFmt='#,##0.00';
  ws.eachRow((row,rowNum)=>{
    if(hasHeader&&rowNum===1)return;
    const isTot=hasTotals&&rowNum===ws.rowCount;
    row.height=18;
    row.eachCell({includeEmpty:true},c=>{
      if(isTot){
        c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};
        c.font={bold:true,size:10};
        c.border={top:{style:'medium',color:{argb:'FF1D3C2A'}},bottom:{style:'thin',color:{argb:'FFE0E0E0'}},right:{style:'thin',color:{argb:'FFE0E0E0'}},left:{style:'thin',color:{argb:'FFE0E0E0'}}};
      } else {
        c.fill={type:'pattern',pattern:'solid',fgColor:{argb:rowNum%2===0?'FFFAFAF8':'FFFFFFFF'}};
        c.font={size:10};
        c.border={bottom:{style:'thin',color:{argb:'FFF0F0F0'}},right:{style:'thin',color:{argb:'FFF5F5F5'}},left:{style:'thin',color:{argb:'FFF5F5F5'}}};
      }
      c.alignment={horizontal:'right',vertical:'middle'};
      if(typeof c.value==='number')c.numFmt=numFmt;
    });
  });
}

// Add title + accountant rows to Excel
function _xlAddTitle(ws,title,cols,summary){
  // IMPORTANT: addRow first, then mergeCells
  ...
  t.getCell(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};
  t.getCell(1).alignment={horizontal:'right',vertical:'middle'};
  ws.getRow(1).height=32;

  // Row 2 — Accountant + Owner
  const a=ws.addRow(['✍ محاسب: محمود مصباح  |  📞 01114892670  |  المهندس محمد شكري  |  📞 01099808939  |  تاريخ: '+new Date().toLocaleDateString('ar-EG')+(summary?'  |  '+summary:'')]);
  ws.mergeCells(`A2:${colLetter}2`);
  a.getCell(1).font={size:10,color:{argb:'FF1D3C2A'},bold:true};
  a.getCell(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFEAF5EE'}};
  a.getCell(1).alignment={horizontal:'right',vertical:'middle'};
  a.getCell(1).border={bottom:{style:'medium',color:{argb:'FF1D3C2A'}}};
...
// Add footer row to Excel
function _xlAddFooter(ws,cols){
  ws.addRow([]);
  const colLetter=String.fromCharCode(64+cols);
  const f=ws.addRow(['Legacy Fine Touch  |  المهندس محمد شكري  |  01099808939  |  محاسب: محمود مصباح  |  01114892670  |  سري وخاص بالشركة']);
  ws.mergeCells(`A${ws.rowCount}:${colLetter}${ws.rowCount}`);
  f.getCell(1).font={size:9,color:{argb:'FF888888'},italic:true};
  f.getCell(1).alignment={horizontal:'center'};
  f.getCell(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF9F9F9'}};
  f.getCell(1).border={top:{style:'thin',color:{argb:'FFE0E0E0'}}};
  f.height=16;
}
...
async function downloadDashPDF(){
  ...
  const rows=d.filtered.sort((a,b)=>parseDt(a.entry_date)-parseDt(b.entry_date)).map(e=>{
    const proj=allProjects.find(p=>p.id===e.project_id);
    const color=e.type==='i'?'${COLORS['primary-btn']}':'${COLORS['danger']}';
    return `<tr>
  ...
  <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;direction:rtl;background:${COLORS['bg-gray']}}
  .page{background:#fff;max-width:900px;margin:0 auto;padding:32px}
  .hdr{display:flex;justify-content:space-between;align-items:center;padding-bottom:18px;border-bottom:3px solid ${COLORS['primary']};margin-bottom:22px}
  .hdr h1{font-size:22px;color:${COLORS['primary']};font-weight:900;margin-bottom:4px}.hdr .sub{font-size:12px;color:${COLORS['text-soft']}}
  .hdr-logo{height:60px;object-fit:contain}
  .kpis{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:24px}
  .kpi{border-radius:10px;padding:14px 16px;text-align:center}.kpi-lbl{font-size:10px;color:${COLORS['text-soft']};margin-bottom:6px}.kpi-val{font-size:20px;font-weight:900}
  table{width:100%;border-collapse:collapse;font-size:11px}
  thead tr{background:${COLORS['primary']}}th{color:${COLORS['accent']};padding:9px 8px;text-align:right;font-size:10px}
  td{padding:7px 8px;border-bottom:1px solid ${COLORS['bg-light']}}tr:nth-child(even) td{background:${COLORS['bg-linen']}}
  .footer{margin-top:24px;padding-top:14px;border-top:2px solid ${COLORS['bg-light']};display:flex;justify-content:space-between;align-items:center}
  .footer-logo{height:30px;opacity:.5}.footer-txt{font-size:10px;color:${COLORS['text-pale']};text-align:center}
  ...
  <div class="kpis">
    <div class="kpi is32"><div class="kpi-lbl">إجمالي الوارد</div><div class="kpi-val" style="color:${COLORS['primary-btn']}">▲ ${fn(d.inc)} ج</div></div>
    <div class="kpi box-err"><div class="kpi-lbl">إجمالي المصروف</div><div class="kpi-val" style="color:${COLORS['danger']}">▼ ${fn(d.exp)} ج</div></div>
    <div class="kpi" style="background:${d.bal>=0?'${COLORS['success-pale']}':'${COLORS['danger-pale']}'};border:1px solid ${d.bal>=0?'${COLORS['success-muted']}':'${COLORS['danger-tint']}'}"><div class="kpi-lbl">صافي الرصيد</div><div class="kpi-val" style="color:${d.bal>=0?'${COLORS['primary-btn']}':'${COLORS['danger']}'}">${d.bal>=0?'▲':'▼'} ${fn(Math.abs(d.bal))} ج</div></div>
  </div>
  <table><thead><tr><th>التاريخ</th><th>النوع</th><th>البند</th><th>البيان</th><th>المبلغ</th><th>المشروع</th></tr></thead>
  <tbody>${rows}</tbody></table>
  <div class="footer"><img src="${LOGO_B64}" class="footer-logo"><div class="footer-txt">Legacy Fine Touch · م. محمد شكري · 01099808939</div><div style="font-size:10px;color:${COLORS['text-pale']}">سري وخاص بالشركة</div></div>
...
function runAdvFilter(){
  ...
  ae.innerHTML=`<div class="is33">
    <span>📅 ${fromStr||'البداية'} → ${toStr||'اليوم'} · ${filtered.length} قيد</span>
    <span style="font-weight:700;color:${COLORS['danger-soft']}">▼ ${fn(total)} ج</span>
  </div>
  ${filtered.length===0?'<div class="emp">لا يوجد مصروفات في هذه الفترة</div>':
    filtered.map(e=>`<div class="rw">
      <span>${e.entry_date||'—'}</span>
      <span>${e.category||'—'}</span>
      <span style="color:${COLORS['text-faint']}">${e.description||''}</span>
      <span style="color:${COLORS['danger-soft']};font-weight:700">▼ ${fn(e.amount)} ج</span>
    </div>`).join('')}`;
}
...
async function downloadAdvPDF(){
  ...
  try{
    ...
    const rows=sorted.map((e,i)=>{
      const proj=allProjects.find(p=>p.id===e.project_id);
      return `<tr>
        <td class="is34">${i+1}</td>
        <td style="font-size:10px">${cleanDate(e.entry_date)}</td>
        <td style="font-weight:700;color:${COLORS['danger']}">${e.category||'—'}</td>
        <td style="color:${COLORS['text-body']}">${e.description||'—'}</td>
        <td style="color:${COLORS['text-soft']};font-size:10px">${proj?.name||'—'}</td>
        <td class="is35">${Number(e.amount).toLocaleString('en-US')} ج</td>
      </tr>`;
    }).join('');
    const instRows=installs.map(i=>`<tr>
      <td>${i.note||'دفعة'}</td>
      <td class="is36">${i.inst_date||'—'}</td>
      <td class="is37">${Number(i.amount).toLocaleString('en-US')} ج</td>
    </tr>`).join('');
    const html=`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>تقرير عهدة - ${curAdv?.person_name}</title>
    <style>
      *{margin:0;padding:0;box-sizing:border-box}
      body{font-family:Arial,sans-serif;direction:rtl;background:${COLORS['bg-gray']}}
      .page{background:#fff;max-width:860px;margin:0 auto;padding:32px;min-height:100vh}
      .hdr{display:flex;justify-content:space-between;align-items:center;padding-bottom:16px;border-bottom:3px solid ${COLORS['primary']};margin-bottom:20px}
      .hdr h1{font-size:20px;color:${COLORS['primary']};font-weight:900;margin-bottom:4px}
      .hdr .sub{font-size:11px;color:${COLORS['text-soft']}}
      ...
      .kpi-lbl{font-size:10px;color:${COLORS['text-soft']};margin-bottom:5px;font-weight:600}
      .kpi-val{font-size:18px;font-weight:900}
      .sec-title{font-size:13px;font-weight:800;color:${COLORS['primary']};padding:8px 0;border-bottom:2px solid ${COLORS['border-light']};margin-bottom:0;margin-top:16px}
      table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:16px}
      thead tr{background:${COLORS['primary']}}
      th{color:${COLORS['accent']};padding:8px;text-align:right;font-size:10px;font-weight:700}
      td{padding:7px 8px;border-bottom:1px solid ${COLORS['bg-light']};vertical-align:middle}
      tr:nth-child(even) td{background:${COLORS['bg-linen']}}
      .footer{margin-top:20px;padding-top:12px;border-top:2px solid ${COLORS['bg-light']};display:flex;justify-content:space-between;align-items:center}
      .footer-logo{height:28px;opacity:.5}
      .footer-txt{font-size:10px;color:${COLORS['text-pale']};text-align:center}
      .print-btn{display:block;margin:0 auto 20px;background:${COLORS['primary']};color:${COLORS['accent']};border:none;padding:10px 30px;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit}
    ...
    <div class="page">
      ...
      <div class="kpis">
        <div class="kpi is38">
          ...
          <div class="kpi-val" style="color:${COLORS['info']}">${Number(totalGiven).toLocaleString('en-US')} ج</div>
        </div>
        <div class="kpi box-err">
          <div class="kpi-lbl">إجمالي الصرف</div>
          <div class="kpi-val" style="color:${COLORS['danger']}">${Number(totalSpent).toLocaleString('en-US')} ج</div>
        </div>
        <div class="kpi" style="background:${remaining>=0?'${COLORS['warning-tint']}':'${COLORS['danger-pale']}'};border:1px solid ${remaining>=0?'${COLORS['warning-muted']}':'${COLORS['danger-tint']}'}">
          <div class="kpi-lbl">الرصيد المتبقي</div>
          <div class="kpi-val" style="color:${remaining>=0?'${COLORS['warning-text']}':'${COLORS['danger']}'}">${Number(remaining).toLocaleString('en-US')} ج</div>
        </div>
      </div>
      ...
      <table>
        ...
        <tbody>${rows}</tbody>
        <tfoot><tr style="background:${COLORS['primary']}"><td colspan="5" class="is39">الإجمالي</td><td class="is40">${Number(totalSpent).toLocaleString('en-US')} ج</td></tr></tfoot>
      </table>
      ...
      <table>
        ...
        <tfoot><tr style="background:${COLORS['info']}"><td colspan="2" class="is41">الإجمالي</td><td class="is42">${Number(totalGiven).toLocaleString('en-US')} ج</td></tr></tfoot>
      </table>`:''}
      <div class="footer">
        ...
        <div style="font-size:10px;color:${COLORS['text-pale']}">سري وخاص بالشركة</div>
      </div>
    </div>
    </body></html>`;
...
function runCashFlow(){
  ...
  if(!ents.length){
    document.getElementById('repCashResult').innerHTML='<div class="is43">لا توجد بيانات في هذه الفترة</div>';
    return;
  }
  ...
  document.getElementById('repCashResult').innerHTML=`
    <div class="cf-kpi-row">
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي الوارد</div><div class="cf-kpi-val" style="color:${COLORS['success-soft']}">+${fn(totalInc)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي المصاريف</div><div class="cf-kpi-val" style="color:${COLORS['danger-soft']}">-${fn(totalExp)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">صافي التدفق</div><div class="cf-kpi-val" style="color:${net>=0?'${COLORS['info-sky']}':'${COLORS['danger-soft']}'}">${net>=0?'+':''}${fn(net)} ج</div></div>
    </div>
      ...
      ${rows.map(r=>{
        ...
        return `<div class="cf-row">
          <div class="cf-row-hdr">
            ...
            <div class="cf-row-net" style="color:${rn>=0?'${COLORS['success-soft']}':'${COLORS['danger-soft']}'}">${rn>=0?'+':''}${fn(rn)} ج</div>
          </div>
  ...
  _loadChartJs(()=>{
    ...
    ctx._chartInst=new Chart(ctx,{
      ...
      options:{
        ...
        scales:{x:{ticks:{color:'${COLORS['text-soft']}',font:{size:11},maxRotation:45},grid:{display:false}},y:{ticks:{color:'${COLORS['text-soft']}',font:{size:10},callback:v=>fn(v)},grid:{color:'rgba(255,255,255,.06)'}}}
      }
    });
  });
}
...
function runSummary(){
  ...
  if(!rows.length){
    document.getElementById('repSummaryResult').innerHTML='<div class="is43">لا توجد بيانات في هذه الفترة</div>';
    return;
  }
  ...
  document.getElementById('repSummaryResult').innerHTML=`
    <div class="is44">${period}</div>
    <div class="cf-kpi-row" style="margin-bottom:20px">
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي الوارد</div><div class="cf-kpi-val" style="color:${COLORS['success-soft']}">+${fn(totInc)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي المصاريف</div><div class="cf-kpi-val" style="color:${COLORS['danger-soft']}">-${fn(totExp)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">الصافي</div><div class="cf-kpi-val" style="color:${totNet>=0?'${COLORS['info-sky']}':'${COLORS['danger-soft']}'}">${totNet>=0?'+':''}${fn(totNet)} ج</div></div>
    </div>
    ...
    ${rows.sort((a,b)=>b.net-a.net).map(r=>`
      <div class="sum-proj-card">
        ...
        <div class="sum-proj-row"><span class="sum-row-lbl">الوارد</span><span class="sum-row-val" style="color:${COLORS['success-soft']}">+${fn(r.inc)} ج</span></div>
        <div class="sum-proj-row"><span class="sum-row-lbl">المصاريف</span><span class="sum-row-val" style="color:${COLORS['danger-soft']}">-${fn(r.exp)} ج</span></div>
        <div class="sum-proj-row"><span class="sum-row-lbl">الصافي</span><span class="sum-row-val" style="color:${r.net>=0?'${COLORS['success-soft']}':'${COLORS['danger-soft']}'};font-size:15px">${r.net>=0?'+':''}${fn(r.net)} ج</span></div>
  ...
  _loadChartJs(()=>{
    ...
    ctx._chartInst=new Chart(ctx,{
      ...
      options:{
        responsive:true,maintainAspectRatio:false,
        plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`${ctx.dataset.label}: ${fn(ctx.parsed.y)} ج`}}},
        scales:{x:{ticks:{color:'${COLORS['text-soft']}',font:{size:11},maxRotation:30},grid:{display:false}},y:{ticks:{color:'${COLORS['text-soft']}',font:{size:10},callback:v=>fn(v)},grid:{color:'rgba(255,255,255,.06)'}}}
      }
    });
  });
}
  ...
  document.getElementById('repResult').innerHTML=`
    <div class="filter-result">
      ...
      <div class="is45">
        ...
        <button class="filter-btn is46" onclick="repExportPDF()">📕 تحميل PDF</button>
        <span class="is31">${sorted.length} قيد إجمالي</span>
      </div>
    ...
    document.getElementById('repAdvResult').innerHTML=`
      <div class="filter-result">
        ...
        <div class="is45">
          ...
          <button class="filter-btn is46" onclick="repAdvExportPDF()">📕 تحميل PDF</button>
        </div>
        <div class="filter-entries" style="max-height:400px">
          ${sorted.map(e=>{
            const proj=allProjects.find(p=>p.id===e.project_id);
...
async function repExportExcel(){
  ...
  try{
    ...
    const summary=`وارد: ${fn(d.inc)} ج  |  مصاريف: ${fn(d.exp)} ج  |  رصيد: ${fn(d.bal)} ج  |  ${d.period}`;
    _xlAddTitle(ws,'تقرير مشروع: '+d.projName,7,summary);
    // Header row
    ws.addRow(hdr);
    ws.lastRow.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFE8E0D0'},bold:true,size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'medium',color:{argb:'FFC9A84C'}}};});
    ws.lastRow.height=22;

    d.filtered.forEach((e,i)=>{
      const proj=allProjects.find(p=>p.id===e.project_id);
      const isI=e.type==='i';
      const row=ws.addRow([cleanDate(e.entry_date)||'',isI?'▲ وارد':'▼ مصروف',proj?.name||'',e.category||'',e.description||'',e.contractor||'',_xlFmt(e.amount)]);
      row.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:i%2===0?'FFFFFFFF':'FFFAFAF8'}};c.font={size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'thin',color:{argb:'FFF0F0F0'}},right:{style:'thin',color:{argb:'FFF5F5F5'}}};if(typeof c.value==='number')c.numFmt='#,##0.00';});
      row.getCell(2).font={color:{argb:isI?'FF1E6B3A':'FF922B21'},bold:true,size:10};
      row.getCell(7).font={color:{argb:isI?'FF1E6B3A':'FF922B21'},bold:true,size:10};
      row.height=18;
    });

    // Totals
    ws.addRow([]);
    const s1=ws.addRow(['','','','','','▲ إجمالي الوارد',_xlFmt(d.inc)]);
    s1.getCell(6).font={bold:true,color:{argb:'FF1E6B3A'}};s1.getCell(7).font={bold:true,color:{argb:'FF1E6B3A'}};s1.getCell(7).numFmt='#,##0.00';
    const s2=ws.addRow(['','','','','','▼ إجمالي المصروف',_xlFmt(d.exp)]);
    s2.getCell(6).font={bold:true,color:{argb:'FF922B21'}};s2.getCell(7).font={bold:true,color:{argb:'FF922B21'}};s2.getCell(7).numFmt='#,##0.00';
    const s3=ws.addRow(['','','','','','الرصيد',_xlFmt(d.bal)]);
    s3.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};});
    s3.getCell(6).font={bold:true,size:11};s3.getCell(7).font={bold:true,color:{argb:d.bal>=0?'FF1E6B3A':'FF922B21'},size:11};s3.getCell(7).numFmt='#,##0.00';
    s3.getCell(6).border={top:{style:'medium',color:{argb:'FF1D3C2A'}}};
    s3.getCell(7).border={top:{style:'medium',color:{argb:'FF1D3C2A'}}};

    _xlAddFooter(ws,7);
    ws.columns=[{width:14},{width:12},{width:20},{width:16},{width:26},{width:18},{width:16}];

    const buf=await wb.xlsx.writeBuffer();
    const a=document.createElement('a');
    a.href=URL.createObjectURL(new Blob([buf],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}));
    a.download='تقرير_'+d.projName+'_'+new Date().toLocaleDateString('en-CA')+'.xlsx';
    a.click();
...
async function repExportPDF(){
  ...
  const rows=d.filtered.map((e,i)=>{
    const proj=allProjects.find(p=>p.id===e.project_id);
    const isI=e.type==='i';
    const c=isI?'${COLORS['primary-btn']}':'${COLORS['danger']}';
    const etLbl={'payment':'💰 دفعة','work':'🔨 أعمال','material':'🔩 مصنعيات'};
    const et=e.entry_type?`<span style="font-size:9px;padding:2px 6px;border-radius:8px;font-weight:700;background:${e.entry_type==='payment'?'${COLORS['success-pale']}':e.entry_type==='work'?'${COLORS['info-bg']}':'${COLORS['warning-pale']}'};color:${e.entry_type==='payment'?'${COLORS['primary-btn']}':e.entry_type==='work'?'${COLORS['info']}':'${COLORS['warning-dark']}'}">${etLbl[e.entry_type]}</span> `:'';
    const creator=e.created_by?(profileMap[e.created_by]||'—'):'غير مسجل';
    return `<tr><td class="is47">${i+1}</td><td style="font-size:10px">${cleanDate(e.entry_date)}</td><td><span style="display:inline-block;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;background:${isI?'${COLORS['success-pale']}':'${COLORS['danger-pale']}'};color:${c}">${isI?'▲ وارد':'▼ مصروف'}</span></td><td style="font-size:10px;color:${COLORS['text-body']}">${proj?.name||''}</td><td style="font-weight:600">${e.category||'—'}</td><td style="color:${COLORS['text-body']}">${et}${e.description||''}</td><td style="font-size:10px;color:${COLORS['text-body']}">${e.contractor||'—'}</td><td class="is48">${creator}</td><td style="color:${c};font-weight:700;white-space:nowrap">${isI?'▲':'▼'} ${fn(e.amount)} ج</td></tr>`;
  }).join('');
  // ملخص المقاولين
  const mqMap={};
  ...
  const mqRows=Object.entries(mqMap).map(([name,m])=>{
    const rem=m.pay-(m.work+m.mat);
    return `<tr><td style="font-weight:700">👷 ${name}</td><td style="text-align:center;color:${COLORS['primary-btn']}">${fn(m.pay)} ج</td><td style="text-align:center;color:${COLORS['info']}">${fn(m.work)} ج</td><td style="text-align:center;color:${COLORS['warning-dark']}">${fn(m.mat)} ج</td><td style="text-align:center;font-weight:900;color:${rem>=0?'${COLORS['primary']}':'${COLORS['danger']}'}">${rem>=0?'':'-'}${fn(Math.abs(rem))} ج</td></tr>`;
  }).join('');
  const mqSection=mqRows?`
    <h3 class="is49">👷 ملخص المقاولين</h3>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:Arial,sans-serif;direction:rtl;color:${COLORS['text-dark']};background:${COLORS['bg-gray']};padding:0}
    .page{background:#fff;max-width:900px;margin:0 auto;padding:32px;min-height:100vh}
    /* HEADER */
    .hdr{display:flex;justify-content:space-between;align-items:center;padding-bottom:18px;border-bottom:3px solid ${COLORS['primary']};margin-bottom:22px}
    .hdr-left h1{font-size:22px;color:${COLORS['primary']};font-weight:900;margin-bottom:4px}
    .hdr-left .sub{font-size:12px;color:${COLORS['text-soft']}}
    .hdr-logo{height:60px;object-fit:contain}
    /* KPIs */
    .kpis{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:24px}
    .kpi{border-radius:10px;padding:14px 16px;text-align:center}
    .kpi-lbl{font-size:10px;color:${COLORS['text-soft']};margin-bottom:6px;letter-spacing:.5px;font-weight:600}
    .kpi-val{font-size:20px;font-weight:900}
    /* TABLE */
    .sec-title{font-size:13px;font-weight:800;color:${COLORS['primary']};padding:8px 0 10px;border-bottom:2px solid ${COLORS['border-light']};margin-bottom:0;display:flex;align-items:center;gap:6px}
    table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:20px}
    thead tr{background:${COLORS['primary']}}
    th{color:${COLORS['accent']};padding:9px 8px;text-align:right;font-size:10px;font-weight:700;letter-spacing:.3px}
    td{padding:7px 8px;border-bottom:1px solid ${COLORS['bg-light']};vertical-align:middle}
    tr:nth-child(even) td{background:${COLORS['bg-linen']}}
    tr:last-child td{border-bottom:none}
    .badge{display:inline-block;padding:2px 7px;border-radius:10px;font-size:9px;font-weight:700}
    .badge-i{background:${COLORS['success-pale']};color:${COLORS['primary-btn']}}
    .badge-e{background:${COLORS['danger-pale']};color:${COLORS['danger']}}
    /* MQ SECTION */
  ...
  <div class="page">
    ...
    <div class="kpis">
      <div class="kpi kpi-inc">
        ...
        <div class="kpi-val">▲ ${fn(d.inc)} ج</div>
      </div>
      <div class="kpi kpi-exp">
        <div class="kpi-lbl">إجمالي المصروف</div>
        <div class="kpi-val">▼ ${fn(d.exp)} ج</div>
      </div>
      <div class="kpi ${d.bal>=0?'kpi-net-pos':'kpi-net-neg'}">
        <div class="kpi-lbl">صافي الرصيد</div>
        <div class="kpi-val">${d.bal>=0?'▲':'▼'} ${fn(Math.abs(d.bal))} ج</div>
      </div>
    </div>
    <div class="sec-title">📒 تفاصيل القيود <span class="is50">(${d.filtered.length} قيد)</span></div>
    <table>
...
function repAdvExportPDF(){
  ...
  const html=_pdfOpen('تقرير عهدة')+
    _pdfHeader('💼 تقرير عهدة','صاحب العهدة: '+d.advName+' · الفترة: '+d.period)+
      ...
      <table class="is52">
        <thead>
          <tr style="background:${COLORS['primary']}">
            <th class="is53">المشروع</th>
            <th class="is53">المقاول</th>
        ...
        <tbody>
          ${rows.map((r,i)=>`<tr style="background:${i%2===0?'#fff':'${COLORS['bg-faint']}'};border-bottom:1px solid ${COLORS['warning-cream']}">
            <td class="is54">${r.proj}</td>
            <td style="padding:9px 12px">👷 ${r.mq}</td>
            <td class="is55">${fn(r.work+r.mat)} ج</td>
            <td class="is56">${fn(r.pay)} ج</td>
            <td class="is57">${fn(r.due)} ج</td>
          </tr>`).join('')}
        </tbody>
        <tfoot>
          <tr style="background:${COLORS['bg-latte']}">
            <td colspan="4" class="is58">إجمالي المستحقات</td>
            <td class="is59">${fn(total)} ج</td>
          </tr>
        </tfoot>
      </table>
    </div>`;
...
function cashExportPDF(){
  ...
  if(!rows||!rows.length){alert('اعرض التقرير أولاً');return;}
...
async function cashExportExcel(){
  const rows=window._cashRows;
  if(!rows||!rows.length){alert('اعرض التقرير أولاً');return;}
  ...
  const totExp=rows.reduce((s,r)=>s+r.exp,0);
  const net=totInc-totExp;
  const summary=`وارد: ${fn(totInc)} ج  |  مصاريف: ${fn(totExp)} ج  |  صافي: ${fn(net)} ج`;
  _xlAddTitle(ws,'تقرير التدفق النقدي — Legacy Fine Touch',4,summary);
  // Headers
  ws.addRow(['الشهر','الوارد (ج)','المصاريف (ج)','الصافي (ج)']);
  ws.lastRow.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFE8E0D0'},bold:true,size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'medium',color:{argb:'FFC9A84C'}}};});
  ws.lastRow.height=22;
  // Data
  rows.forEach((r,i)=>{
    const n=r.inc-r.exp;
    const row=ws.addRow([_monthLabel(r.y,r.m),_xlFmt(r.inc),_xlFmt(r.exp),_xlFmt(n)]);
    row.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:i%2===0?'FFFFFFFF':'FFFAFAF8'}};c.font={size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'thin',color:{argb:'FFF0F0F0'}},right:{style:'thin',color:{argb:'FFF5F5F5'}}};if(typeof c.value==='number')c.numFmt='#,##0.00';});
    row.getCell(2).font={color:{argb:'FF1E6B3A'},size:10};
    row.getCell(3).font={color:{argb:'FF922B21'},size:10};
    row.getCell(4).font={color:{argb:n>=0?'FF1E6B3A':'FF922B21'},bold:true,size:10};
    row.height=18;
  });
  // Totals
  const tot=ws.addRow(['الإجمالي',_xlFmt(totInc),_xlFmt(totExp),_xlFmt(net)]);
  tot.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};c.font={bold:true,size:11};c.border={top:{style:'medium',color:{argb:'FF1D3C2A'}}};c.alignment={horizontal:'right',vertical:'middle'};if(typeof c.value==='number')c.numFmt='#,##0.00';});
  tot.getCell(2).font={bold:true,size:11,color:{argb:'FF1E6B3A'}};
  tot.getCell(3).font={bold:true,size:11,color:{argb:'FF922B21'}};
  tot.getCell(4).font={bold:true,size:11,color:{argb:net>=0?'FF1E6B3A':'FF922B21'}};
  _xlAddFooter(ws,4);
  ws.columns=[{width:24},{width:20},{width:20},{width:20}];
  const buf=await wb.xlsx.writeBuffer();
...
async function summaryExportExcel(){
  const rows=window._summaryRows;
  if(!rows||!rows.length){alert('اعرض التقرير أولاً');return;}
  ...
  const totExp=rows.reduce((s,r)=>s+r.exp,0);
  const totNet=totInc-totExp;
  _xlAddTitle(ws,'الملخص الدوري للمشاريع — Legacy Fine Touch',5,`وارد: ${fn(totInc)} ج  |  مصاريف: ${fn(totExp)} ج  |  صافي: ${fn(totNet)} ج`);
  // Headers
  ws.addRow(['المشروع','الوارد (ج)','المصاريف (ج)','الصافي (ج)','القيود']);
  ws.lastRow.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFE8E0D0'},bold:true,size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'medium',color:{argb:'FFC9A84C'}}};});
  ws.lastRow.height=22;
  // Data
  rows.sort((a,b)=>b.net-a.net).forEach((r,i)=>{
    const row=ws.addRow([r.name,_xlFmt(r.inc),_xlFmt(r.exp),_xlFmt(r.net),r.count]);
    row.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:i%2===0?'FFFFFFFF':'FFFAFAF8'}};c.font={size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'thin',color:{argb:'FFF0F0F0'}},right:{style:'thin',color:{argb:'FFF5F5F5'}}};if(typeof c.value==='number')c.numFmt='#,##0.00';});
    row.getCell(1).font={bold:true,size:10};
    row.getCell(2).font={color:{argb:'FF1E6B3A'},size:10};
    row.getCell(3).font={color:{argb:'FF922B21'},size:10};
    row.getCell(4).font={color:{argb:r.net>=0?'FF1E6B3A':'FF922B21'},bold:true,size:10};
    row.height=18;
  });
  // Totals
  const tot=ws.addRow(['الإجمالي',_xlFmt(totInc),_xlFmt(totExp),_xlFmt(totNet),rows.reduce((s,r)=>s+r.count,0)]);
  tot.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};c.font={bold:true,size:11};c.border={top:{style:'medium',color:{argb:'FF1D3C2A'}}};c.alignment={horizontal:'right',vertical:'middle'};if(typeof c.value==='number')c.numFmt='#,##0.00';});
  tot.getCell(2).font={bold:true,size:11,color:{argb:'FF1E6B3A'}};
  tot.getCell(3).font={bold:true,size:11,color:{argb:'FF922B21'}};
  tot.getCell(4).font={bold:true,size:11,color:{argb:totNet>=0?'FF1E6B3A':'FF922B21'}};
  _xlAddFooter(ws,5);
  ws.columns=[{width:28},{width:18},{width:18},{width:18},{width:12}];
  const buf=await wb.xlsx.writeBuffer();
  const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([buf],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}));
...
async function duesExportExcel(){
  ...
  const total=window._duesTotal||0;
  _xlAddTitle(ws,'تقرير مستحقات المقاولين — Legacy Fine Touch',5,`إجمالي المستحقات: ${fn(total)} ج`);
  // Headers
  ws.addRow(['المشروع','المقاول','أعمال + مصنعيات (ج)','مدفوع (ج)','المستحق (ج)']);
  ws.lastRow.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFE8E0D0'},bold:true,size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'medium',color:{argb:'FFC9A84C'}}};});
  ws.lastRow.height=22;
  window._duesRows.forEach((r,i)=>{
    const row=ws.addRow([r.proj,'👷 '+r.mq,_xlFmt(r.work+r.mat),_xlFmt(r.pay),_xlFmt(r.due)]);
    row.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:i%2===0?'FFFFFFFF':'FFFAFAF8'}};c.font={size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'thin',color:{argb:'FFF0F0F0'}},right:{style:'thin',color:{argb:'FFF5F5F5'}}};if(typeof c.value==='number')c.numFmt='#,##0.00';});
    row.getCell(1).font={bold:true,size:10};
    row.getCell(4).font={color:{argb:'FF1E6B3A'},size:10};
    row.getCell(5).font={bold:true,color:{argb:'FF922B21'},size:10};
    row.height=18;
  });
  // Totals
  const tot=ws.addRow(['إجمالي المستحقات','','','',_xlFmt(total)]);
  tot.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFFFEBEE'}};c.font={bold:true,size:11};c.border={top:{style:'medium',color:{argb:'FF922B21'}}};c.alignment={horizontal:'right',vertical:'middle'};if(typeof c.value==='number')c.numFmt='#,##0.00';});
  tot.getCell(5).font={bold:true,size:12,color:{argb:'FF922B21'}};
  _xlAddFooter(ws,5);
  ws.columns=[{width:22},{width:20},{width:22},{width:16},{width:18}];
  const buf=await wb.xlsx.writeBuffer();
  const blob=new Blob([buf],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
...
function mqPrintReport(idx){
  ...
  const trs=rows.map((e,i)=>`
    <tr>
      <td style="text-align:center;color:${COLORS['text-soft']}">${i+1}</td>
      <td style="text-align:center">${e.entry_date||'—'}</td>
      <td>${e.entry_type?etLbl[e.entry_type]||'—':'—'}</td>
      <td>${e.category||'—'}</td>
      <td>${e.description||'—'}</td>
      <td class="is63">${fn2(e.amount)} ج</td>
    </tr>`).join('');
...
async function saveMqPay(){
  ...
  if(isNaN(a)||a<=0){msg.textContent='⚠️ ادخل المبلغ';msg.style.color='${COLORS['danger-alt']}';return;}
  if(!cat){msg.textContent='⚠️ ادخل البند';msg.style.color='${COLORS['danger-alt']}';return;}
  msg.textContent='⏳ جاري الحفظ...';msg.style.color='${COLORS['text-soft']}';
  ...
  try{
    if(uRole==='admin'){
      ...
      await loadEntries();
      allEntries=allEntries.filter(e=>e.project_id!==curPid).concat(entries);refreshProjSummary(curPid);
      msg.textContent='✅ تم الحفظ';msg.style.color='${COLORS['primary-btn']}';
      setTimeout(()=>{closeMqPay();rp();},800);
    }else{
      const pending={...entry,status:'pending',submitted_by:uid,submitted_at:new Date().toISOString()};
      await sb('pending_entries','POST',pending);
      msg.textContent='⏳ في انتظار موافقة الأدمن';msg.style.color='${COLORS['warning-text']}';
      setTimeout(()=>closeMqPay(),1200);
    }
  }catch(e){setSav('❌ خطأ: '+e.message,'er');msg.style.color='${COLORS['danger-alt']}';}
}
...
function setCatView(v,btn){
  ...
  ['cvList','cvMq'].forEach(id=>{
    ...
    if(b){b.style.background='#fff';b.style.color='${COLORS['text-hint']}';b.style.borderColor='${COLORS['border-warm']}';}
  });
  if(btn){btn.style.background='${COLORS['info-bg']}';btn.style.color='${COLORS['info']}';btn.style.borderColor='${COLORS['info']}';}
}
...
function renderMqOpts(q){
  ...
  if(!html)html='<div class="is29">لا يوجد مقاولين بعد</div>';
  list.innerHTML=html;
}
...
function showAdvImModal(){
  ...
  document.getElementById('advImBody').innerHTML=advImRows.map((r,i)=>{
    ...
    const bg=i%2===0?'#fff':'${COLORS['bg-warm2']}';
    const hasPrj=!!r.pid;
    return `<tr style="background:${bg}">
      <td class="is64">${i+1}</td>
      <td class="is65">${fn(r.amt)} ج</td>
      <td style="padding:8px 12px"><span class="is66">${r.cat||'—'}</span></td>
      <td class="is67">${r.desc||'—'}</td>
      <td class="is68">${r.dt||'—'}</td>
      <td class="is69">${r.mq||'—'}</td>
      <td style="padding:8px 12px">
        <select data-idx="${i}" onchange="advImRows[this.dataset.idx].pid=this.value;this.style.borderColor=this.value?'${COLORS['primary-mid']}':'${COLORS['warning-alt']}';this.style.background=this.value?'#fff':'${COLORS['warning-faint']}'"
          style="width:100%;padding:6px 8px;border:1.5px solid ${hasPrj?'${COLORS['primary-mid']}':'${COLORS['warning-alt']}'};border-radius:6px;font-size:12px;font-family:inherit;background:${hasPrj?'#fff':'${COLORS['warning-faint']}'};color:${COLORS['primary']}">
          ${opts}
        </select>
      </td>
    </tr>`;
...
function setSearchMode(m){
  ...
  ['seq','name'].forEach(x=>{
    ...
    const on=x===m;
    btn.style.background=on?'${COLORS['primary']}':'#fff';
    btn.style.color=on?'${COLORS['accent']}':'${COLORS['text-muted']}';
    btn.style.borderColor=on?'${COLORS['primary']}':'${COLORS['border']}';
...
async function searchBySeq(){
  ...
  if(!seq||isNaN(seq)){div.innerHTML='<div class="is70">⚠️ أدخل رقم القيد</div>';return;}
  div.innerHTML='<div class="is71">⏳ جاري البحث...</div>';
  try{
    const data=await sb('entries?or=(seq.eq.'+seq+',entry_no.eq.'+seq+')&order=created_at');
    if(!data||!data.length){div.innerHTML='<div class="is72">❌ القيد <strong>'+seq+'</strong> غير موجود</div>';return;}
    const projMap={};allProjects.forEach(p=>projMap[p.id]=p.name);
    const profMap=await getProfileMap();
    div.innerHTML=data.map((e,i)=>buildSearchCard(e,i,data.length,projMap,profMap)).join('');
  }catch(er){div.innerHTML='<div class="is73">❌ '+er.message+'</div>';}
}
async function searchByName(){
  const q=document.getElementById('searchNameInp').value.trim();
  const div=document.getElementById('searchResult');
  if(!q){div.innerHTML='<div class="is70">⚠️ أدخل كلمة للبحث</div>';return;}
  div.innerHTML='<div class="is71">⏳ جاري البحث...</div>';
  ...
  if(!total){div.innerHTML='<div class="is72">❌ لا نتائج لـ &quot;'+q+'&quot;</div>';return;}
  const projMap={};allProjects.forEach(p=>projMap[p.id]=p.name);
  const profMap=await getProfileMap();
  div.innerHTML='<div class="is74">📊 '+total+' نتيجة'+(total>50?' (أول 50)':'')+'</div>'+results.map((e,i)=>buildSearchCard(e,i,results.length,projMap,profMap)).join('');
}
...
function buildSearchCard(e,idx,total,projMap,profMap){
  ...
  const topInfo=total>1?'<div class="is75">نتيجة '+(idx+1)+' من '+total+'</div>':'';
  const catVal=(e.category||'').replace(/"/g,'&quot;');
  const mqVal=(e.contractor||'').replace(/"/g,'&quot;');
  const descVal=(e.description||'').replace(/"/g,'&quot;');
  return '<div id="sc-'+e.id+'" class="is76">'+
    '<div class="is77">'+
      '<div><div class="is10">قيد رقم '+(e.seq||'?')+' — '+projName+'</div>'+
      '<div class="is78">✍️ أدخله: '+creator+'</div></div>'+
      '<span class="is79">'+(isInc?'⬆ وارد':'⬇ مصروف')+'</span>'+
    '</div>'+
    '<div style="padding:14px 16px">'+topInfo+
      '<div class="is80">'+
        '<div><label class="lbl-md">📁 المشروع</label>'+
          '<select id="se-proj-'+e.id+'" class="is81">'+projOpts+'</select></div>'+
        '<div><label class="lbl-md">📊 النوع</label>'+
          '<select id="se-type-'+e.id+'" class="is81">'+
        ...
        '<div><label class="lbl-md">💰 المبلغ (ج)</label>'+
          '<input id="se-amt-'+e.id+'" type="number" step="any" value="'+e.amount+'" class="is82"></div>'+
        '<div><label class="lbl-md">📅 التاريخ</label>'+
          '<input id="se-date-'+e.id+'" type="date" value="'+dtVal+'" class="is83"></div>'+
        '<div><label class="lbl-md">🏷️ البند</label>'+
          '<input id="se-cat-'+e.id+'" type="text" value="'+catVal+'" list="cl" class="is83"></div>'+
        '<div><label class="lbl-md">👷 المقاول</label>'+
          '<input id="se-mq-'+e.id+'" type="text" value="'+mqVal+'" class="is84"></div>'+
        '<div style="grid-column:1/-1"><label class="lbl-md">📝 البيان</label>'+
          '<input id="se-desc-'+e.id+'" type="text" value="'+descVal+'" class="is83"></div>'+
      '</div>'+
      '<div style="display:flex;gap:8px">'+
        '<button onclick="saveSearchEdit(&quot;'+e.id+'&quot;)" class="is85">💾 حفظ التعديلات</button>'+
        '<button onclick="deleteSearchEntry(&quot;'+e.id+'&quot;,'+(e.seq||0)+')" class="is86">🗑 حذف</button>'+
      '</div>'+
      '<div id="se-msg-'+e.id+'" class="is87"></div>'+
    '</div></div>';
}
async function saveSearchEdit(id){
  const msgEl=document.getElementById('se-msg-'+id);
  msgEl.innerHTML='⏳ جاري الحفظ...';msgEl.style.color='${COLORS['text-soft']}';
  ...
  if(isNaN(updates.amount)){msgEl.innerHTML='❌ أدخل مبلغ صحيح';msgEl.style.color='${COLORS['danger']}';return;}
  try{
    ...
    msgEl.innerHTML='✅ تم الحفظ';msgEl.style.color='${COLORS['primary-btn']}';
    setSav('✅ تم تعديل القيد','ok');
    setTimeout(()=>{msgEl.innerHTML='';},3000);
  }catch(er){msgEl.innerHTML='❌ '+er.message;msgEl.style.color='${COLORS['danger']}';}
}
async function deleteSearchEntry(id,seq){
  ...
  try{
    ...
    setSav('✅ تم الحذف','ok');
    setTimeout(()=>{const r=document.getElementById('searchResult');if(r&&!r.querySelector('[id^="sc-"]'))r.innerHTML='<div class="is88">✅ تم حذف القيد</div>';},400);
...
async function loadApprovals(){
  ...
  if(!el)return;
  el.innerHTML='<div class="is89">⏳ جاري التحميل...</div>';
  try{
    ...
    if(!hasEntries&&!hasAdv){
      el.innerHTML='<div class="is90">🎉 لا يوجد قيود في الانتظار</div>';
      return;
    }
    const projMap={};allProjects.forEach(p=>projMap[p.id]=p.name);
    ...
    // ── قيود المشاريع ──
    if(hasEntries){
      html+=`<div class="is91">📋 قيود المشاريع (${entRows.length})</div>`;
      html+=entRows.map(r=>{
        const proj=projMap[r.project_id]||'—';
        const typeLabel=r.type==='i'
          ?'<span class="is92">📤 وارد</span>'
          :'<span class="is93">📥 مصروف</span>';
        return `<div class="is94">
          <div class="is95">
            <div class="is96">
              ${typeLabel}
              <span class="title-sm">${fn(r.amount)} ج</span>
              ${r.category?`<span class="is97">${r.category}</span>`:''}
            </div>
            <span style="font-size:10px;color:${COLORS['text-faint']}">${r.submitted_at?r.submitted_at.substring(0,16).replace('T',' '):'—'}</span>
          </div>
          <div class="is98">
            ${r.description?`<span>📝 ${r.description}</span> &nbsp;`:''}
            ${r.contractor?`<span>👷 ${r.contractor}</span> &nbsp;`:''}
            <span style="color:${COLORS['text-soft']}">🏗️ ${proj}</span> &nbsp;
            <span style="color:${COLORS['text-soft']}">📅 ${r.entry_date||'—'}</span>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button onclick="approveEntry('${r.id}')" class="is99">✅ موافقة</button>
            <button onclick="editAndApproveEntry('${r.id}')" class="is100">✏️ تعديل وموافقة</button>
            <button onclick="rejectEntry('${r.id}')" class="is101">❌ رفض</button>
          </div>
        </div>`;
      }).join('');
    }
    ...
    // ── العهد ──
    if(hasAdv){
      html+=`<div class="is102">💼 العهد والدفعات (${advRows.length})</div>`;
      html+=advRows.map(r=>{
        const isAdv=r.type==='advance';
        const label=isAdv
          ?'<span class="is103">💼 عهدة جديدة</span>'
          :'<span class="is104">💰 دفعة</span>';
        const detail=isAdv
          ?`<span class="title-sm">${r.person_name||'—'}</span>${r.notes?` <span style="color:${COLORS['text-soft']};font-size:12px">· ${r.notes}</span>`:''}`
          :`<span class="title-sm">${fn(r.amount)} ج</span> <span style="color:${COLORS['text-soft']};font-size:12px">لـ ${advMap[r.advance_id]||'—'}</span> <span style="color:${COLORS['text-faint']};font-size:11px">· ${r.inst_note||'دفعة'}</span>`;
        return `<div class="is94">
          <div class="is95">
            <div class="is96">${label} ${detail}</div>
            <span style="font-size:10px;color:${COLORS['text-faint']}">${r.submitted_at?r.submitted_at.substring(0,16).replace('T',' '):'—'}</span>
          </div>
          <div style="display:flex;gap:8px">
            <button onclick="approveAdv('${r.id}')" class="is105">✅ موافقة</button>
            <button onclick="rejectAdv('${r.id}')" class="is106">❌ رفض</button>
          </div>
        </div>`;
      }).join('');
    }
    el.innerHTML=html;
  }catch(e){el.innerHTML='<div style="color:${COLORS['danger']};padding:20px">❌ خطأ: '+e.message+'</div>';}
}
...
async function editAndApproveEntry(id){
  try{
    ...
    const inp='width:100%;padding:10px;border:1.5px solid ${COLORS['border-light']};border-radius:10px;font-family:inherit;font-size:13px;margin-bottom:12px;box-sizing:border-box';
    ...
    ov.innerHTML=`
      <div class="is107">
        <div class="modal-hdr">
          ...
          <button onclick="document.getElementById('eaModal').remove()" class="is108">✕</button>
        </div>
        <label class="lbl-lg">🏗️ المشروع</label>
        <select id="eaProjId" style="${inp}background:${COLORS['bg-pure']}">
          ${projOptions}
        </select>
        <label class="lbl-lg">النوع</label>
        <select id="eaType" style="${inp}background:${COLORS['bg-pure']}">
...
function renderNpOnline(){
  ...
  body.innerHTML=sorted.map(u=>{
    ...
    const lastSeenTxt=u.is_online?'<span style="color:${COLORS['success-soft']};font-size:10px">● متصل الآن</span>':(u.last_seen?`آخر ظهور: ${notifFmtTime(u.last_seen)}`:'لم يتصل بعد');
    return `<div class="ou-card">
      <div class="ou-avatar">${initials}<span class="ou-status ${u.is_online?'on':'off'}"></span></div>
      <div style="flex:1;min-width:0">
        <div class="ou-name">${u.name||'—'}${u.id===uid?' <span class="is110">(أنت)</span>':''}</div>
