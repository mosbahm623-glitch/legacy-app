...
(function(){
  ...
  function createPicker(){
    ...
    d.id='dp-popup';
    d.style.cssText='position:fixed;z-index:99999;background:#ffffff;border:1.5px solid #d0d0d0;border-radius:14px;box-shadow:0 8px 40px rgba(0,0,0,.25);padding:14px;width:280px;font-family:inherit;direction:rtl;display:none;';
    // dark mode
    const _dpApplyTheme=()=>{const dark=document.body.classList.contains('dark-mode');d.style.background=dark?'#1e2a1e':'#ffffff';d.style.borderColor=dark?'rgba(212,196,154,.3)':'#d0d0d0';d.style.color=dark?'#e8e0d0':'#1a1a1a';};
    _dpApplyTheme();
    const _dpObs=new MutationObserver(_dpApplyTheme);
    _dpObs.observe(document.body,{attributes:true,attributeFilter:['class']});
    d.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px"><button id="dp-prev" style="background:none;border:none;cursor:pointer;font-size:22px;color:#555;padding:2px 8px;line-height:1">&#8250;</button><span id="dp-label" style="font-weight:700;color:#1a1a1a;font-size:14px"></span><button id="dp-next" style="background:none;border:none;cursor:pointer;font-size:22px;color:#555;padding:2px 8px;line-height:1">&#8249;</button></div><div id="dp-hdr" style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px;margin-bottom:4px"></div><div id="dp-grid" style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px"></div><div style="margin-top:10px;display:flex;justify-content:center"><button id="dp-today" style="background:#1D3C2A;color:#fff;border:none;border-radius:8px;padding:6px 22px;cursor:pointer;font-size:12px;font-family:inherit">اليوم</button></div>`;
    document.body.appendChild(d);
    DAYS_AR.forEach(day=>{
      const c=document.createElement('div');
      c.style.cssText='text-align:center;font-size:10px;color:#888;font-weight:700;padding:3px 0';
      c.textContent=day;
  ...
  function renderDP(){
    ...
    for(let d2=1;d2<=days;d2++){
      ...
      const isSel=selD===d2;
      btn.textContent=d2;
      btn.style.cssText='border:none;cursor:pointer;border-radius:7px;padding:6px 2px;font-size:12px;font-family:inherit;width:100%;transition:background .1s;'+(isSel?'background:#1D3C2A;color:#fff;font-weight:700;':(isToday?'background:#F5EFD7;color:#1D3C2A;font-weight:700;':'background:none;color:#1a1a1a;'));
      btn.onmouseenter=function(){if(!isSel)this.style.background='#f0f4f0';};
      btn.onmouseleave=function(){if(!isSel)this.style.background=isToday?'#F5EFD7':'none';};
...
function initDateInput(el){
  ...
  // منع الشفافية — background واضح
  el.classList.add('dp-input');
  // لو مفيش value، placeholder واضح
  if(!el.placeholder)el.placeholder='dd/mm/yyyy';
  el.addEventListener('click',function(e){e.stopPropagation();window._openDP(this);});
}
    ...
    '<div style="padding:14px 16px">'+topInfo+
      '<div class="is80">'+
        ...
        '<div><label class="lbl-md">📅 التاريخ</label>'+
          '<input id="se-date-'+e.id+'" type="text" value="'+dtVal+'" class="is83 dp-input" readonly style="cursor:pointer"></div>'+
