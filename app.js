...
function setSav(m,c){
  ...
  if(!el)return;
  el.className='sav-badge '+(c==='ok'?'ok':c==='ng'?'ng':'er');
  const txt=el.querySelector('#savText');
  if(txt)txt.textContent=c==='ok'?'متصل':c==='ng'?'تحذير':'خطأ';
}
...
async function initApp(){
  ...
  const navBackup=document.getElementById('navBackupBtn');
  const navExport=document.getElementById('navExportBtn');
  if(navBackup)navBackup.style.display=uRole==='admin'?'block':'none';
  if(navExport)navExport.style.display=uRole==='admin'?'block':'none';
...
function showScreen(s){
  ...
  curScreen=s;
  ['dash','daily','proj','adv','admin','rep','search','approvals'].forEach(x=>{
...
async function loadDailyLog(){
  ...
  container.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px">
    <div style="background:#EAF5EE;border-radius:10px;padding:12px;text-align:center"><div style="font-size:9px;color:#1E6B3A;margin-bottom:4px">وارد اليوم</div><div style="font-size:16px;font-weight:800;color:#1E6B3A">+${fn(totalInc)} ج</div></div>
...
async function exportAllProjects(){
  const btn=document.getElementById('navExportBtn');
  if(!btn)return;
  btn.disabled=true;
  btn.textContent='⏳';
...
async function loadDashboard(){
  try{
    ...
    // الإجماليات من الملخصات الجاهزة — مش بنلف على كل قيد
    let totalInc=0,totalExp=0;
    allProjects.forEach(p=>{
    ...
    // كروت المشاريع من الملخصات الجاهزة
    ...
    initDashFilter();
    const projColors=['#6FCF97','#56CCF2','#F2C94C','#BB86FC','#F7845A','#6FCF97','#FF8A80','#82B1FF','#6FCF97','#FFD180'];
    const catColors={'كهربا':'#F2C94C','خرسانة':'#BB86FC','سباكة':'#56CCF2','نجارة':'#6FCF97','تشطيب':'#F7845A','دهانات':'#6FCF97','عمالة':'#FFD180','حديد':'#9E9E9E','بناء':'#6FCF97','إشراف':'#D4C49A','أثاث':'#FF8A80','نقاشة':'#82B1FF'};

    // ترتيب من الملخصات الجاهزة
    const sortedProjects=[...allProjects].sort((a,b)=>{
      const ba=(projSummaries[a.id]||{balDirect:0}).balDirect;
      const bb=(projSummaries[b.id]||{balDirect:0}).balDirect;
      return bb-ba;
    });
    container.className='proj-grid';
    container.innerHTML=sortedProjects.map((p,idx)=>{
      const color=projColors[idx%projColors.length];
      const s=projSummaries[p.id]||{inc:0,exp:0,expDirect:0,bal:0,balDirect:0,cats:[],count:0};
      ...
      return `<div class="pcard" onclick="showScreen('proj');setTimeout(()=>{document.getElementById('ps').value='${p.id}';sw('${p.id}');},100)" style="animation-delay:${idx*0.07}s">
        <div class="pc-head" style="background:linear-gradient(135deg,${color}10 0%,transparent 55%)">
          <div class="pc-mood">
            <span class="pc-mood-badge ${moodCls}">${moodTxt}</span>
            <span style="font-size:9px;color:rgba(212,196,154,.3)">${s.count} قيد</span>
          </div>
        ...
        <div class="tl-group">
          ...
          ${items.map(e=>{
            ...
            return `<div class="tl-item">
              ...
              <div class="tl-info">
                <div class="tl-main"><span class="tl-proj-tag">${proj?.name||'—'}</span>${e.category||'—'}</div>
...
async function loadAllProjects(){
  ...
  [allProjects,allEntries]=await Promise.all([
    sb('projects?order=created_at'),
    sb('entries?select=id,project_id,type,amount,category,description,contractor,entry_date,created_at,advance_id,seq&order=created_at.desc')
  ]);
...
function confirmWithPassword(actionLabel, actionIcon, onConfirmed){
  ...
  ov.innerHTML=`
    <div style="background:#fff;border-radius:18px;padding:28px 24px;width:100%;max-width:360px;font-family:inherit;direction:rtl;box-shadow:0 24px 64px rgba(0,0,0,.4)">
      <div style="text-align:center;margin-bottom:20px">
        <div style="font-size:32px;margin-bottom:8px">${actionIcon}</div>
        <div style="font-size:15px;font-weight:800;color:#1D3C2A">${actionLabel}</div>
        <div style="font-size:12px;color:#888;margin-top:4px">أدخل كلمة المرور للتأكيد</div>
      </div>
      <div style="position:relative;margin-bottom:8px">
        <input id="pwcInput" type="password" placeholder="كلمة المرور"
          style="width:100%;padding:12px 14px;border:1.5px solid #ddd;border-radius:12px;font-family:inherit;font-size:14px;box-sizing:border-box;transition:border .2s"
          onfocus="this.style.borderColor='#1D3C2A'" onblur="this.style.borderColor='#ddd'"
          onkeydown="if(event.key==='Enter')submitPwConfirm()">
      </div>
      <div id="pwcMsg" style="font-size:12px;color:#922B21;text-align:center;min-height:18px;margin-bottom:14px"></div>
      <div style="display:flex;gap:10px">
        <button id="pwcOkBtn" onclick="submitPwConfirm()" style="flex:1;background:#1D3C2A;color:#D4C49A;border:none;padding:13px;border-radius:12px;font-family:inherit;font-size:14px;font-weight:800;cursor:pointer">✔ تأكيد</button>
        <button onclick="document.getElementById('pwConfirmModal').remove()" style="background:#f0f0f0;color:#555;border:none;padding:13px 18px;border-radius:12px;font-family:inherit;font-size:14px;cursor:pointer">إلغاء</button>
      </div>
    </div>`;
...
async function sw(pid){
  curPid=pid;cTab='s';setSav('⏳...','ng');
  await loadEntries();setSav('☁️ متصل','ok');
  // التاريخ دايما اليوم لما تفتح مشروع
...
function editProject(){
  ...
  ov.innerHTML=`
    <div style="background:#fff;border-radius:18px;padding:24px;width:100%;max-width:400px;font-family:inherit;direction:rtl;box-shadow:0 20px 60px rgba(0,0,0,.35)">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <div style="font-size:16px;font-weight:800;color:#1D3C2A">✏️ تعديل المشروع</div>
        <button onclick="document.getElementById('editProjModal').remove()" style="background:#f0f0f0;border:none;border-radius:8px;padding:6px 10px;cursor:pointer;font-size:13px">✕</button>
      </div>
      <label style="font-size:12px;color:#555;display:block;margin-bottom:5px;font-weight:600">اسم المشروع</label>
      <input id="epName" type="text" value="${(p.name||'').replace(/"/g,'&quot;')}"
        style="width:100%;padding:11px 14px;border:1.5px solid #ddd;border-radius:11px;font-family:inherit;font-size:14px;margin-bottom:14px;transition:border .2s"
        onfocus="this.style.borderColor='#1D3C2A'" onblur="this.style.borderColor='#ddd'">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px">
        <div>
          <label style="font-size:12px;color:#555;display:block;margin-bottom:5px;font-weight:600">📅 تاريخ البداية</label>
          <input id="epStart" type="text" value="${p.start_date||''}" placeholder="dd/mm/yyyy"
            style="width:100%;padding:10px 12px;border:1.5px solid #ddd;border-radius:11px;font-family:inherit;font-size:13px;transition:border .2s"
            onfocus="this.style.borderColor='#1D3C2A'" onblur="this.style.borderColor='#ddd'">
        </div>
        <div>
          <label style="font-size:12px;color:#555;display:block;margin-bottom:5px;font-weight:600">📅 تاريخ الإغلاق</label>
          <input id="epClose" type="text" value="${p.close_date||''}" placeholder="dd/mm/yyyy"
            style="width:100%;padding:10px 12px;border:1.5px solid #ddd;border-radius:11px;font-family:inherit;font-size:13px;transition:border .2s"
            onfocus="this.style.borderColor='#1D3C2A'" onblur="this.style.borderColor='#ddd'">
        </div>
      </div>
      <div id="epMsg" style="font-size:12px;margin-bottom:12px;min-height:18px;text-align:center"></div>
      <div style="display:flex;gap:10px">
        <button onclick="saveProjectEdit()" style="flex:1;background:#1D3C2A;color:#D4C49A;border:none;padding:13px;border-radius:12px;font-family:inherit;font-size:14px;font-weight:800;cursor:pointer">💾 حفظ التعديلات</button>
        <button onclick="document.getElementById('editProjModal').remove()" style="background:#f0f0f0;color:#555;border:none;padding:13px 18px;border-radius:12px;font-family:inherit;font-size:14px;cursor:pointer">إلغاء</button>
      </div>
    </div>`;
  document.body.appendChild(ov);
  ov.addEventListener('click',e=>{if(e.target===ov)ov.remove();});
...
async function saveProjectEdit(){
  ...
  if(!name){msg.style.color='#922B21';msg.textContent='❌ الاسم مطلوب';return;}
  msg.style.color='#856404';msg.textContent='⏳ جاري الحفظ...';
  try{
    ...
    msg.style.color='#1E6B3A';msg.textContent='✅ تم الحفظ';
    setSav('✅ تم تعديل المشروع','ok');
    setTimeout(()=>document.getElementById('editProjModal')?.remove(),700);
  }catch(e){msg.style.color='#922B21';msg.textContent='❌ خطأ: '+e.message;}
}
...
function re(){
  ...
  if(cTab==='m'){
    ...
    el.innerHTML=mqs.map((m,idx)=>{
      ...
      const rows=m.rows.sort((a,b)=>pdt(b.entry_date)-pdt(a.entry_date)).map(e=>{
        ...
        const etBg={'payment':'#EAF5EE','work':'#E3F0FF','material':'#FFF3E0'};
  ...
  html+=es.map(e=>{
    ...
    const etBg=e.entry_type==='payment'?'#EAF5EE':e.entry_type==='work'?'#E3F0FF':e.entry_type==='material'?'#FFF3E0':'';
  ...
  if(hasMqTypes){
    ...
    html+=Object.entries(mqMap).map(([name,d])=>{
      ...
      const rows=d.rows.map(e=>{
        ...
        const etBg=e.entry_type==='payment'?'#EAF5EE':e.entry_type==='work'?'#E3F0FF':'#FFF3E0';
...
async function loadAdvList(){
  ...
  try{
    ...
    al.innerHTML=advances.map(a=>{
      ...
      const remColor=rem<0?'#922B21':rem===0?'#1E6B3A':'#856404';
      return '<div class="adv-card" onclick="openAdv(\''+a.id+'\')"><div class="adv-card-h"><div class="adv-name">👤 '+a.person_name+ownerBadge+'</div><span class="adv-status '+(a.status==='open'?'open':'closed')+'">'+(a.status==='open'?'⏳ مفتوحة':'✅ مغلقة')+'</span></div>'+(a.notes?'<div style="font-size:10px;color:#888;margin-top:4px">'+a.notes+'</div>':'')+'<div class="adv-nums"><div class="adv-num"><div class="adv-num-l">العهدة</div><div class="adv-num-v" style="color:#185FA5">'+fn(totalGiven)+'</div></div><div class="adv-num"><div class="adv-num-l">صرف</div><div class="adv-num-v" style="color:#922B21">'+fn(spent)+'</div></div><div class="adv-num"><div class="adv-num-l">الباقي</div><div class="adv-num-v" style="color:'+remColor+'">'+fn(rem)+'</div></div></div><div style="background:#eee;border-radius:10px;height:4px;margin-top:8px"><div style="background:#1D3C2A;width:'+pct+'%;height:4px;border-radius:10px"></div></div></div>';
    }).join('');
  ...
  try{
    ...
    if(installs.length===0){
      ...
      il.innerHTML=installs.map(ins=>{
        var db=uRole!=='viewer'?`<button class='db' onclick='delInstall("${ins.id}")'>&#3A6B52;</button>`:'';
    ...
    if(advEntries.length===0&&pendingAdvEntries.length===0){
      ...
      const approvedHtml=advEntries.map(e2=>{
        ...
        var db2=canEditAdv?`<button class='db' onclick='editAdvEntry("${e2.id}")' style='color:#1D3C2A'>&#9998;</button><button class='db' onclick='delAdvEntry("${e2.id}")'>&#3A6B52;</button>`:'';
      ...
      const pendingHtml=pendingAdvEntries.map(e2=>{
        var pName=projMap[e2.project_id]||'&mdash;';
        return `<div class='rw' style='opacity:.75;border:1px dashed #F2C94C;background:#FFFDE7'><div class='ri'><div class='rd'>&#x23f3; ${e2.description||'&mdash;'} <span style='font-size:10px;color:#856404;background:#FFF3CD;padding:1px 6px;border-radius:8px'>في الانتظار</span></div><div class='rm'>${pName} &middot; ${e2.category||'&mdash;'} &middot; ${cleanDate(e2.entry_date)}</div></div><div style='display:flex;align-items:center'><div class='ra neg' style='color:#856404'>${fn(e2.amount)} &#x62c;</div></div></div>`;
      }).join('');
...
function renderCatOpts(q){
  ...
  if(cats.length){
    html+=cats.map(({c,n})=>`<div class="cat-opt" onclick="selectCat('${c.replace(/'/g,"\\'")}')">
      ...
      <span style="font-size:9px;opacity:.4;background:rgba(0,0,0,.06);padding:1px 6px;border-radius:10px">${n}×</span>
    </div>`).join('');
  }
  ...
  if(!html)html='<div style="padding:12px;text-align:center;font-size:11px;color:#aaa">لا توجد بنود بعد<br><small>اكتب بند جديد وسيُضاف تلقائياً</small></div>';
  list.innerHTML=html;
}
...
function initRealtime(){
  ...
  try{
    ...
    _rtAdvCh=_sbc.channel('advances-all')
      ...
      .subscribe();
  }catch(e){console.log('initRealtime err:',e);}
}
...
function checkPwdStrength(v){
  ...
  const pct=score*20;
  const colors=['#EB5757','#F39C12','#F39C12','#6FCF97','#5CB87A'];
  bar.style.width=pct+'%';
...
async function downloadDashPDF(){
  ...
  <div class="kpis">
    <div class="kpi" style="background:#EAF5EE;border:1px solid #6FCF97"><div class="kpi-lbl">إجمالي الوارد</div><div class="kpi-val" style="color:#1E6B3A">▲ ${fn(d.inc)} ج</div></div>
    ...
    <div class="kpi" style="background:${d.bal>=0?'#EAF5EE':'#FFEBEE'};border:1px solid ${d.bal>=0?'#6FCF97':'#FFCDD2'}"><div class="kpi-lbl">صافي الرصيد</div><div class="kpi-val" style="color:${d.bal>=0?'#1E6B3A':'#922B21'}">${d.bal>=0?'▲':'▼'} ${fn(Math.abs(d.bal))} ج</div></div>
  </div>
...
async function downloadAdvReport(){
  ...
  try{
    ...
    const GREEN='FF1D3C2A', BEIGE='FFD4C49A', LGREY='FFF5F5F5', WHITE='FFFFFFFF';
    const LIGHT_GREEN='FFE8F5ED', LIGHT_RED='FFFAE5E5';
    ...
    // ── عرض الأعمدة ──
    ws.columns=[
      {width:14},{width:18},{width:28},{width:22},{width:16}
    ];

    // ── سطر العنوان ──
    ws.mergeCells('A1:E1');
    const titleCell=ws.getCell('A1');
    titleCell.value='💼 تقرير عهدة: '+(curAdv?.person_name||'—');
    titleCell.font={bold:true,size:14,color:{argb:BEIGE},name:'Arial'};
    ...
    dateCell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF9F7F2'}};
    ...
    ['A4','B4','C4'].forEach((addr,i)=>{
      const labels=['إجمالي الدفعات','إجمالي الصرف','الرصيد المتبقي'];
      const values=[totalGiven,totalSpent,remaining];
      const colors=['FF185FA5','FF922B21',remaining>=0?'FF856404':'FF1E6B3A'];
      ws.mergeCells(addr+':'+addr);
      const c=ws.getCell(addr);
      c.value=labels[i]+': '+Number(values[i]).toLocaleString('en-US')+' ج';
      c.font={bold:true,size:11,color:{argb:colors[i]},name:'Arial'};
      c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF9F7F2'}};
      c.alignment={horizontal:'center',vertical:'middle'};
      c.border={bottom:{style:'medium',color:{argb:GREEN}}};
    });

    // ── هيدر الجدول ──
    ws.getRow(5).height=14;
    const hdrs=['التاريخ','البند','البيان','المشروع','المبلغ (ج)'];
    ...
    hdrs.forEach((h,i)=>{
      ...
      c.border={bottom:{style:'medium',color:{argb:BEIGE}}};
    });
    ...
    sorted.forEach((e,idx)=>{
      ...
      const vals=[e.entry_date||'—',e.category||'—',e.description||'—',proj?.name||'—',e.amount];
      vals.forEach((v,i)=>{
        const c=ws.getCell(rowNum,i+1);
        c.value=v;
        c.font={size:10,name:'Arial'};
        c.fill={type:'pattern',pattern:'solid',fgColor:{argb:bg}};
        c.alignment={horizontal:i===4?'center':'right',vertical:'middle',readingOrder:'rightToLeft'};
        c.border={bottom:{style:'thin',color:{argb:'FFEEEEEE'}}};
        if(i===4){c.numFmt='#,##0';c.font={...c.font,bold:true,color:{argb:'FF922B21'}};}
      });
    });

    // ── سطر الإجمالي ──
    const totRow=7+sorted.length+1;
    ws.getRow(totRow).height=26;
    ws.mergeCells('A'+totRow+':D'+totRow);
    const totLbl=ws.getCell('A'+totRow);
    totLbl.value='إجمالي المصروفات';
    totLbl.font={bold:true,size:11,color:{argb:WHITE},name:'Arial'};
    totLbl.fill={type:'pattern',pattern:'solid',fgColor:{argb:GREEN}};
    totLbl.alignment={horizontal:'center',vertical:'middle'};
    const totVal=ws.getCell('E'+totRow);
    totVal.value=totalSpent;
    totVal.numFmt='#,##0';
    totVal.font={bold:true,size:12,color:{argb:BEIGE},name:'Arial'};
    totVal.fill={type:'pattern',pattern:'solid',fgColor:{argb:GREEN}};
...
async function downloadAdvPDF(){
  ...
  try{
    ...
    const rows=sorted.map((e,i)=>{
      const proj=allProjects.find(p=>p.id===e.project_id);
      return `<tr>
        <td style="color:#aaa;text-align:center;font-size:10px">${i+1}</td>
        ...
        <td style="color:#888;font-size:10px">${proj?.name||'—'}</td>
        <td style="font-weight:800;color:#922B21;text-align:center">${Number(e.amount).toLocaleString('en-US')} ج</td>
      </tr>`;
    }).join('');
    const instRows=installs.map(i=>`<tr>
      <td>${i.note||'دفعة'}</td>
      <td style="text-align:center;color:#888;font-size:10px">${i.inst_date||'—'}</td>
      <td style="font-weight:800;color:#185FA5;text-align:center">${Number(i.amount).toLocaleString('en-US')} ج</td>
    </tr>`).join('');
    const html=`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>تقرير عهدة - ${curAdv?.person_name}</title>
    <style>
      *{margin:0;padding:0;box-sizing:border-box}
      body{font-family:Arial,sans-serif;direction:rtl;background:#f5f5f5}
      .page{background:#fff;max-width:860px;margin:0 auto;padding:32px;min-height:100vh}
      .hdr{display:flex;justify-content:space-between;align-items:center;padding-bottom:16px;border-bottom:3px solid #1D3C2A;margin-bottom:20px}
      .hdr h1{font-size:20px;color:#1D3C2A;font-weight:900;margin-bottom:4px}
      .hdr .sub{font-size:11px;color:#888}
      .hdr-logo{height:55px;object-fit:contain}
      .kpis{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:22px}
      .kpi{border-radius:10px;padding:12px 14px;text-align:center}
      .kpi-lbl{font-size:10px;color:#888;margin-bottom:5px;font-weight:600}
      .kpi-val{font-size:18px;font-weight:900}
      .sec-title{font-size:13px;font-weight:800;color:#1D3C2A;padding:8px 0;border-bottom:2px solid #e0e0e0;margin-bottom:0;margin-top:16px}
      table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:16px}
      thead tr{background:#1D3C2A}
      th{color:#D4C49A;padding:8px;text-align:right;font-size:10px;font-weight:700}
      td{padding:7px 8px;border-bottom:1px solid #f0f0f0;vertical-align:middle}
      tr:nth-child(even) td{background:#fafaf8}
      .footer{margin-top:20px;padding-top:12px;border-top:2px solid #f0f0f0;display:flex;justify-content:space-between;align-items:center}
      .footer-logo{height:28px;opacity:.5}
      .footer-txt{font-size:10px;color:#bbb;text-align:center}
      .print-btn{display:block;margin:0 auto 20px;background:#1D3C2A;color:#D4C49A;border:none;padding:10px 30px;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit}
      @media print{.print-btn{display:none}body{background:#fff}.page{padding:16px;max-width:100%}}
    </style></head><body>
    <div class="page">
      <button class="print-btn" onclick="window.print()">🖨️ طباعة / حفظ PDF</button>
      <div class="hdr">
        <div>
          <h1>💼 تقرير عهدة: ${curAdv?.person_name||'—'}</h1>
          <div class="sub">🗓 ${now} &nbsp;·&nbsp; ${sorted.length} قيد مصروف &nbsp;·&nbsp; Legacy Fine Touch</div>
        </div>
        <img src="${LOGO_B64}" class="hdr-logo">
      </div>
      <div class="kpis">
        <div class="kpi" style="background:#E3F0FF;border:1px solid #BBDEFB">
          <div class="kpi-lbl">إجمالي الدفعات</div>
          <div class="kpi-val" style="color:#185FA5">${Number(totalGiven).toLocaleString('en-US')} ج</div>
        </div>
        <div class="kpi" style="background:#FFEBEE;border:1px solid #FFCDD2">
          <div class="kpi-lbl">إجمالي الصرف</div>
          <div class="kpi-val" style="color:#922B21">${Number(totalSpent).toLocaleString('en-US')} ج</div>
        </div>
        <div class="kpi" style="background:${remaining>=0?'#FFF8E1':'#FFEBEE'};border:1px solid ${remaining>=0?'#FFE082':'#FFCDD2'}">
          <div class="kpi-lbl">الرصيد المتبقي</div>
          <div class="kpi-val" style="color:${remaining>=0?'#856404':'#922B21'}">${Number(remaining).toLocaleString('en-US')} ج</div>
        </div>
      </div>
      <div class="sec-title">💸 مصروفات العهدة (${sorted.length} قيد)</div>
      <table>
        <thead><tr><th>#</th><th>التاريخ</th><th>البند</th><th>البيان</th><th>المشروع</th><th>المبلغ</th></tr></thead>
        <tbody>${rows}</tbody>
        <tfoot><tr style="background:#1D3C2A"><td colspan="5" style="color:#D4C49A;font-weight:700;padding:8px;text-align:center">الإجمالي</td><td style="color:#D4C49A;font-weight:900;text-align:center;padding:8px">${Number(totalSpent).toLocaleString('en-US')} ج</td></tr></tfoot>
      </table>
      ${installs.length?`<div class="sec-title">💰 الدفعات (${installs.length} دفعة)</div>
      <table>
        <thead><tr><th>البيان</th><th>التاريخ</th><th>المبلغ</th></tr></thead>
        <tbody>${instRows}</tbody>
        <tfoot><tr style="background:#185FA5"><td colspan="2" style="color:#fff;font-weight:700;padding:8px;text-align:center">الإجمالي</td><td style="color:#fff;font-weight:900;text-align:center;padding:8px">${Number(totalGiven).toLocaleString('en-US')} ج</td></tr></tfoot>
      </table>`:''}
      <div class="footer">
    ...
    // تحميل كـ HTML file قابل للطباعة والمشاركة
    const blob=new Blob([html],{type:'text/html;charset=utf-8'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');
    a.href=url;
    a.download='تقرير-عهدة-'+(curAdv?.person_name||'report')+'-'+new Date().toLocaleDateString('ar-EG').replace(/\//g,'-')+'.html';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    setTimeout(()=>URL.revokeObjectURL(url),1000);
    setSav('✅ تم التحميل — افتح الملف واضغط Ctrl+P لتصديره PDF','ok');
  }catch(e){setSav('❌ خطأ: '+e.message,'er');}
}
...
function openReport(type){
  ...
  const titles={cash:'💰 التدفق النقدي',summary:'📋 الملخص الدوري',proj:'🏗️ تقرير المشاريع',adv:'💼 تقرير العهد',dues:'⚠️ مستحقات المقاولين'};
  document.getElementById('repViewTitle').textContent=titles[type]||'';
  ['repCashPanel','repSummaryPanel','repProjPanel','repAdvPanel','repDuesPanel'].forEach(id=>{
...
function runCashFlow(){
  ...
  const toD=toVal?new Date(toVal+' 23:59:59'):null;
  ...
  if(!ents.length){
    document.getElementById('repCashResult').innerHTML='<div style="text-align:center;padding:40px;color:rgba(212,196,154,.35);font-size:13px">لا توجد بيانات في هذه الفترة</div>';
    return;
  }
  ...
  document.getElementById('repCashResult').innerHTML=`
    <div class="cf-kpi-row">
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي الوارد</div><div class="cf-kpi-val" style="color:#6FCF97">+${fn(totalInc)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي المصاريف</div><div class="cf-kpi-val" style="color:#EB5757">-${fn(totalExp)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">صافي التدفق</div><div class="cf-kpi-val" style="color:${net>=0?'#56CCF2':'#EB5757'}">${net>=0?'+':''}${fn(net)} ج</div></div>
    </div>
    <div class="cf-bars">
      ${rows.map(r=>{
        ...
        return `<div class="cf-row">
          <div class="cf-row-hdr">
            ...
            <div class="cf-row-net" style="color:${rn>=0?'#6FCF97':'#EB5757'}">${rn>=0?'+':''}${fn(rn)} ج</div>
          </div>
...
function runSummary(){
  ...
  if(!rows.length){
    document.getElementById('repSummaryResult').innerHTML='<div style="text-align:center;padding:40px;color:rgba(212,196,154,.35);font-size:13px">لا توجد بيانات في هذه الفترة</div>';
    return;
  }
  ...
  document.getElementById('repSummaryResult').innerHTML=`
    <div style="margin-bottom:14px;font-size:11px;color:rgba(212,196,154,.45);font-weight:500">${period}</div>
    <div class="cf-kpi-row" style="margin-bottom:20px">
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي الوارد</div><div class="cf-kpi-val" style="color:#6FCF97">+${fn(totInc)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">إجمالي المصاريف</div><div class="cf-kpi-val" style="color:#EB5757">-${fn(totExp)} ج</div></div>
      <div class="cf-kpi"><div class="cf-kpi-lbl">الصافي</div><div class="cf-kpi-val" style="color:${totNet>=0?'#56CCF2':'#EB5757'}">${totNet>=0?'+':''}${fn(totNet)} ج</div></div>
    </div>
    ${rows.sort((a,b)=>b.net-a.net).map(r=>`
      <div class="sum-proj-card">
        <div class="sum-proj-name">${r.name}</div>
        <div class="sum-proj-row"><span class="sum-row-lbl">الوارد</span><span class="sum-row-val" style="color:#6FCF97">+${fn(r.inc)} ج</span></div>
        <div class="sum-proj-row"><span class="sum-row-lbl">المصاريف</span><span class="sum-row-val" style="color:#EB5757">-${fn(r.exp)} ج</span></div>
        <div class="sum-proj-row"><span class="sum-row-lbl">الصافي</span><span class="sum-row-val" style="color:${r.net>=0?'#6FCF97':'#EB5757'};font-size:15px">${r.net>=0?'+':''}${fn(r.net)} ج</span></div>
...
async function repExportPDF(){
  ...
  const rows=d.filtered.map((e,i)=>{
    ...
    const et=e.entry_type?`<span style="font-size:9px;padding:2px 6px;border-radius:8px;font-weight:700;background:${e.entry_type==='payment'?'#EAF5EE':e.entry_type==='work'?'#E3F0FF':'#FFF3E0'};color:${e.entry_type==='payment'?'#1E6B3A':e.entry_type==='work'?'#185FA5':'#E65100'}">${etLbl[e.entry_type]}</span> `:'';
    const creator=e.created_by?(profileMap[e.created_by]||'—'):'غير مسجل';
    return `<tr><td style="color:#aaa;font-size:10px;text-align:center">${i+1}</td><td style="font-size:10px">${cleanDate(e.entry_date)}</td><td><span style="display:inline-block;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;background:${isI?'#EAF5EE':'#FFEBEE'};color:${c}">${isI?'▲ وارد':'▼ مصروف'}</span></td><td style="font-size:10px;color:#555">${proj?.name||''}</td><td style="font-weight:600">${e.category||'—'}</td><td style="color:#555">${et}${e.description||''}</td><td style="font-size:10px;color:#555">${e.contractor||'—'}</td><td style="font-size:10px;color:#185FA5;font-weight:600">${creator}</td><td style="color:${c};font-weight:700;white-space:nowrap">${isI?'▲':'▼'} ${fn(e.amount)} ج</td></tr>`;
  }).join('');
  // ملخص المقاولين
  const mqMap={};
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .badge-i{background:#EAF5EE;color:#1E6B3A}
  ...
  <div class="page">
    ...
    <div class="kpis">
      <div class="kpi" style="background:#EAF5EE;border:1px solid #6FCF97">
      ...
      <div class="kpi" style="background:${d.bal>=0?'#EAF5EE':'#FFEBEE'};border:1px solid ${d.bal>=0?'#6FCF97':'#FFCDD2'}">
...
<div class="footer">Legacy Fine Touch — تقرير حساب المقاول</div>
...
async function saveMqPay(){
  ...
  }catch(e){setSav('❌ خطأ: '+e.message,'er');msg.style.color='#e74c3c';}
}
...
function getProjectMqs(q){
  const projEntries=curPid
    ? allEntries.filter(e=>e.project_id===curPid&&e.contractor)
    : allEntries.filter(e=>e.contractor);
  const freq={};
...
function renderMqOpts(q){
  ...
  if(!html)html='<div style="padding:12px;text-align:center;font-size:11px;color:#aaa">لا يوجد مقاولين بعد</div>';
  list.innerHTML=html;
}
...
function showAdvImModal(){
  ...
  document.getElementById('advImBody').innerHTML=advImRows.map((r,i)=>{
    ...
    return `<tr style="background:${bg}">
      ...
      <td style="padding:8px 12px"><span style="background:#F4F7F5;color:#1D3C2A;padding:2px 8px;border-radius:4px;font-size:12px">${r.cat||'—'}</span></td>
      ...
      <td style="padding:8px 12px">
        <select data-idx="${i}" onchange="advImRows[this.dataset.idx].pid=this.value;this.style.borderColor=this.value?'#2A5040':'#e0a800';this.style.background=this.value?'#fff':'#fffbf0'"
          style="width:100%;padding:6px 8px;border:1.5px solid ${hasPrj?'#2A5040':'#e0a800'};border-radius:6px;font-size:12px;font-family:inherit;background:${hasPrj?'#fff':'#fffbf0'};color:#1D3C2A">
          ${opts}
        </select>
      </td>
    </tr>`;
...
async function deleteSearchEntry(id,seq){
  ...
  try{
    ...
    setSav('✅ تم الحذف','ok');
    setTimeout(()=>{const r=document.getElementById('searchResult');if(r&&!r.querySelector('[id^="sc-"]'))r.innerHTML='<div style="background:#EAF5EE;color:#1E6B3A;padding:14px 16px;border-radius:10px;text-align:center">✅ تم حذف القيد</div>';},400);
    ...
    if(hasEntries){
      ...
      html+=entRows.map(r=>{
        ...
        const typeLabel=r.type==='i'
          ?'<span style="background:#EAF5EE;color:#1E6B3A;padding:2px 8px;border-radius:8px;font-size:11px;font-weight:700">📤 وارد</span>'
        ...
        return `<div style="background:#fff;border-radius:14px;padding:16px;margin-bottom:10px;box-shadow:0 2px 10px rgba(0,0,0,.07);border:1px solid #eee">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button onclick="approveEntry('${r.id}')" style="flex:1;min-width:80px;background:#1D3C2A;color:#D4C49A;border:none;padding:9px;border-radius:10px;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer">✅ موافقة</button>
            <button onclick="editAndApproveEntry('${r.id}')" style="flex:1;min-width:80px;background:#E3F2FD;color:#1565C0;border:1px solid #90CAF9;padding:9px;border-radius:10px;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer">✏️ تعديل وموافقة</button>
            <button onclick="rejectEntry('${r.id}')" style="flex:1;min-width:80px;background:#FAE5E5;color:#922B21;border:1px solid #f5c6c6;padding:9px;border-radius:10px;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer">❌ رفض</button>
          </div>
        </div>`;
      }).join('');
    }
...
async function editAndApproveEntry(id){
  try{
    ...
    const inp='width:100%;padding:10px;border:1.5px solid #e0e0e0;border-radius:10px;font-family:inherit;font-size:13px;margin-bottom:12px;box-sizing:border-box';
    ...
    ov.innerHTML=`
      <div style="background:#fff;border-radius:18px;padding:24px;width:100%;max-width:440px;max-height:92vh;overflow-y:auto;font-family:inherit;direction:rtl;box-shadow:0 24px 64px rgba(0,0,0,.4)">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
          <div style="font-size:15px;font-weight:800;color:#1D3C2A">✏️ تعديل القيد قبل الموافقة</div>
          <button onclick="document.getElementById('eaModal').remove()" style="background:#f0f0f0;border:none;border-radius:8px;padding:5px 9px;cursor:pointer;font-size:13px">✕</button>
        </div>
        <label style="font-size:12px;color:#555;font-weight:600;display:block;margin-bottom:5px">🏗️ المشروع</label>
        <select id="eaProjId" style="${inp}background:#f9f9f9">
          ${projOptions}
        </select>
        <label style="font-size:12px;color:#555;font-weight:600;display:block;margin-bottom:5px">النوع</label>
        <select id="eaType" style="${inp}background:#f9f9f9">
          <option value="e" ${r.type==='e'?'selected':''}>📥 مصروف</option>
          <option value="i" ${r.type==='i'?'selected':''}>📤 وارد</option>
        </select>
        <label style="font-size:12px;color:#555;font-weight:600;display:block;margin-bottom:5px">المبلغ</label>
        <input id="eaAmount" type="number" value="${r.amount||''}" style="${inp}">
        <label style="font-size:12px;color:#555;font-weight:600;display:block;margin-bottom:5px">البيان</label>
        <input id="eaDesc" type="text" value="${(r.description||'').replace(/"/g,'&quot;')}" style="${inp}">
        <label style="font-size:12px;color:#555;font-weight:600;display:block;margin-bottom:5px">البند</label>
        <input id="eaCat" type="text" value="${(r.category||'').replace(/"/g,'&quot;')}" style="${inp}">
        <label style="font-size:12px;color:#555;font-weight:600;display:block;margin-bottom:5px">المقاول</label>
        <input id="eaContr" type="text" value="${(r.contractor||'').replace(/"/g,'&quot;')}" style="${inp}">
        <label style="font-size:12px;color:#555;font-weight:600;display:block;margin-bottom:5px">التاريخ</label>
        <input id="eaDate" type="text" value="${r.entry_date||''}" placeholder="dd/mm/yyyy" style="${inp}margin-bottom:20px">
        <div style="display:flex;gap:10px">
          <button onclick="confirmEditApprove('${id}')" style="flex:1;background:#1D3C2A;color:#D4C49A;border:none;padding:13px;border-radius:12px;font-family:inherit;font-size:14px;font-weight:800;cursor:pointer">✅ حفظ وموافقة</button>
          <button onclick="document.getElementById('eaModal').remove()" style="background:#f0f0f0;color:#555;border:none;padding:13px 18px;border-radius:12px;font-family:inherit;font-size:14px;cursor:pointer">إلغاء</button>
        </div>
      </div>`;
    document.body.appendChild(ov);
    ov.addEventListener('click',e=>{if(e.target===ov)ov.remove();});
...
function renderNpNotifs(){
  ...
  if(!body)return;
  if(!notifList.length){body.innerHTML='<div class="np-empty">🔔<br>لا توجد إشعارات بعد</div>';return;}
  body.innerHTML=`<button class="np-clear" onclick="clearAllNotifs()">🗑 مسح الكل</button>`+
    notifList.map(n=>`<div class="ni${!n.read?' unread':''}" ${n.pid?`onclick="goToEntry('${n.pid}','${n.eid||''}')" style="cursor:pointer"`:''}>
      <div class="ni-icon">${n.icon}</div>
      <div class="ni-body">
        <div class="ni-title">${n.title}</div>
        ${n.sub?`<div class="ni-sub">${n.sub}</div>`:''}
        ${n.pid?`<div class="ni-time" style="color:rgba(212,196,154,.45);margin-top:3px">اضغط للانتقال للقيد ←</div>`:''}
        <div class="ni-time">${notifFmtTime(n.ts)}</div>
      </div>
    </div>`).join('');
...
function renderNpOnline(){
  ...
  body.innerHTML=sorted.map(u=>{
    ...
    const lastSeenTxt=u.is_online?'<span style="color:#6FCF97;font-size:10px">● متصل الآن</span>':(u.last_seen?`آخر ظهور: ${notifFmtTime(u.last_seen)}`:'لم يتصل بعد');
    return `<div class="ou-card">
      <div class="ou-avatar">${initials}<span class="ou-status ${u.is_online?'on':'off'}"></span></div>
      <div style="flex:1;min-width:0">
        <div class="ou-name">${u.name||'—'}${u.id===uid?' <span style="font-size:10px;color:rgba(212,196,154,.4)">(أنت)</span>':''}</div>
...
async function getUserName(userId){
  ...
  try{await refreshOnlineUsers();}catch(e){}
...
function setupNotifRealtime(){
  ...
  if(uRole==='admin'){
    _rtPendNotifCh=_sbc.channel('notif-pending')
      ...
      .subscribe((status)=>{console.log('Pending notif realtime:',status);});
  }
}
