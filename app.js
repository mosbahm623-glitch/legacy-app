    ...
    if(donutCanvas&&window.Chart){
      ...
      donutCanvas._chartInst=new Chart(donutCanvas,{
        ...
        data:{
          ...
          datasets:[{data:[excellent||0,needFollow||0,critical||0],backgroundColor:['var(--success-light)','var(--accent-gold)','var(--danger-pop)'],borderWidth:0,hoverOffset:4}]
        },
        options:{
...
function _renderCashFlowChart(entries){
  ...
  cashCanvas._chartInst=new Chart(cashCanvas,{
    ...
    data:{
      labels:days,
      datasets:[
        {label:'الوارد',data:incData,borderColor:'var(--success-light)',backgroundColor:'rgba(111,207,151,0.08)',tension:0.4,fill:true,pointRadius:2,borderWidth:2,pointHoverRadius:5},
        {label:'المصروفات',data:expData,borderColor:'var(--danger-pop)',backgroundColor:'rgba(235,87,87,0.06)',tension:0.4,fill:true,pointRadius:2,borderWidth:2,pointHoverRadius:5}
      ]
    },
    options:{
      responsive:true,maintainAspectRatio:false,
...
async function loadAdvList(){
  // إظهار فورم الإضافة للكل — الأدمن يضيف مباشرة، غيره يطلب موافقة
  const newAdvForm=document.getElementById('newAdvForm');
  if(newAdvForm)newAdvForm.style.display='block';
...
async function createAdv(){
  ...
  try{
    if(uRole==='admin'){
      ...
      if(!a||!a[0])throw new Error('لم يتم الحفظ');
      advances.push(a[0]);
      setSav('✅ تم إضافة العهدة','ok');
    }else{
      const res=await sb('pending_advances','POST',{
        type:'advance',
        person_name:name,
        notes:notes||'',
        adv_user_id:uid,
      ...
      if(!res||!res[0])throw new Error('لم يتم الإرسال');
      setSav('⏳ تم الإرسال — في انتظار موافقة الأدمن','ng');
    }
    ...
    console.error('createAdv error:',e);
    setSav('❌ خطأ: '+(e.message||'تعذر الحفظ'),'er');
    notify('❌ '+(e.message||'تعذر الحفظ'),'err');
  }
}
