...
const _PDF_CSS=`
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* ── FOOTER ── */
  .ftr{margin-top:28px;padding-top:14px;border-top:2px solid #eeeeee;display:flex;justify-content:space-between;align-items:center}
  .ftr-logo{height:32px;opacity:.4}
  .ftr-txt{font-size:10px;color:#bbb;text-align:center;line-height:1.6}
  .ftr-conf{font-size:10px;color:#ccc;text-align:left}
...
function _pdfHeader(title,subtitle){
  return `<div class="hdr"><div class="hdr-left"><h1>${title}</h1><div class="sub">${subtitle}</div></div><img src="${LOGO_B64}" class="hdr-logo"></div>`;
}
function _pdfFooter(){
  const now=new Date().toLocaleDateString('ar-EG',{year:'numeric',month:'long',day:'numeric'});
  return `<div class="ftr"><img src="${LOGO_B64}" class="ftr-logo"><div class="ftr-txt">Legacy Fine Touch · م. محمد شكري · 01099808939<br>تم الإنشاء: ${now}</div><div class="ftr-conf">سري وخاص بالشركة</div></div>`;
}
...
function _xlStyle(ws,headers,totalRow){
  // Header row
  const h=ws.getRow(1);
  h.eachCell(c=>{
    c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};
    c.font={color:{argb:'FFE8E0D0'},bold:true,size:10};
  ...
  ws.eachRow((row,rowNum)=>{
    if(rowNum===1)return;
    const isTot=totalRow&&rowNum===ws.rowCount;
    row.height=18;
    row.eachCell(c=>{
      if(isTot){
        c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};
        c.font={bold:true,size:10};
        c.border={top:{style:'medium',color:{argb:'FF1D3C2A'}}};
      } else {
        c.fill={type:'pattern',pattern:'solid',fgColor:{argb:rowNum%2===0?'FFFAFAF8':'FFFFFFFF'}};
        c.font={size:10};
      }
      c.alignment={horizontal:'right',vertical:'middle'};
      c.border={...c.border,bottom:{style:'thin',color:{argb:'FFF0F0F0'}}};
    });
  });
}

// Format number for Excel display
function _xlFmt(n){return typeof n==='number'?n:parseFloat(n)||0;}
...
async function repExportExcel(){
  ...
  try{
    ...
    // Title row
    ws.mergeCells('A1:G1');
    const titleRow=ws.addRow(['تقرير مشروع: '+d.projName+' | '+d.period]);
    titleRow.getCell(1).font={bold:true,size:14,color:{argb:'FFFFFFFF'}};
    titleRow.getCell(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};
    titleRow.getCell(1).alignment={horizontal:'right',vertical:'middle'};
    ws.getRow(1).height=30;
    // Date row
    ws.mergeCells('A2:G2');
    const dRow=ws.addRow(['تاريخ الإنشاء: '+new Date().toLocaleDateString('ar-EG')+' | وارد: '+fn(d.inc)+' ج | مصروف: '+fn(d.exp)+' ج | رصيد: '+fn(d.bal)+' ج']);
    dRow.getCell(1).font={size:10,color:{argb:'FF888888'}};
    dRow.getCell(1).alignment={horizontal:'right'};
    ws.getRow(2).height=18;
    ws.addRow([]);
    // Header row
    const hRow=ws.addRow(hdr);
    hRow.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFE8E0D0'},bold:true,size:10};c.alignment={horizontal:'right',vertical:'middle'};});
    ws.getRow(4).height=22;

    d.filtered.forEach((e,i)=>{
      const proj=allProjects.find(p=>p.id===e.project_id);
      const row=ws.addRow([cleanDate(e.entry_date)||'',e.type==='i'?'▲ وارد':'▼ مصروف',proj?.name||'',e.category||'',e.description||'',e.contractor||'',_xlFmt(e.amount)]);
      const isI=e.type==='i';
      row.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:i%2===0?'FFFFFFFF':'FFFAFAF8'}};c.font={size:10};c.alignment={horizontal:'right',vertical:'middle'};c.border={bottom:{style:'thin',color:{argb:'FFF0F0F0'}}};});
    ...
    s1.getCell(6).font={bold:true,color:{argb:'FF1E6B3A'}};s1.getCell(7).font={bold:true,color:{argb:'FF1E6B3A'}};
    const s2=ws.addRow(['','','','','','▼ إجمالي المصروف',_xlFmt(d.exp)]);
    s2.getCell(6).font={bold:true,color:{argb:'FF922B21'}};s2.getCell(7).font={bold:true,color:{argb:'FF922B21'}};
    const s3=ws.addRow(['','','','','','الرصيد',_xlFmt(d.bal)]);
    s3.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};});
    s3.getCell(6).font={bold:true,size:11};s3.getCell(7).font={bold:true,color:{argb:d.bal>=0?'FF1E6B3A':'FF922B21'},size:11};
...
async function cashExportExcel(){
  ...
  // Title
  ws.mergeCells('A1:D1');
  const t=ws.addRow(['تقرير التدفق النقدي — Legacy Fine Touch']);
  ...
  ws.getRow(1).height=30;
  ws.mergeCells('A2:D2');
  const d2=ws.addRow(['وارد: '+fn(totInc)+' ج | مصاريف: '+fn(totExp)+' ج | صافي: '+fn(net)+' ج | '+new Date().toLocaleDateString('ar-EG')]);
  d2.getCell(1).font={size:10,color:{argb:'FF888888'}};d2.getCell(1).alignment={horizontal:'right'};
  ws.getRow(2).height=18;
  ws.addRow([]);
  // Headers
  ws.addRow(['الشهر','الوارد (ج)','المصاريف (ج)','الصافي (ج)']);
  ws.getRow(4).eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFE8E0D0'},bold:true,size:10};c.alignment={horizontal:'right',vertical:'middle'};});
  ws.getRow(4).height=22;
  // Data
  rows.forEach((r,i)=>{
    const n=r.inc-r.exp;
  ...
  tot.eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};c.font={bold:true,size:11};c.border={top:{style:'medium',color:{argb:'FF1D3C2A'}}};c.alignment={horizontal:'right',vertical:'middle'};});
...
async function summaryExportExcel(){
  ...
  // Title row
  ws.mergeCells('A1:E1');
  const t=ws.addRow(['الملخص الدوري للمشاريع — Legacy Fine Touch']);
  t.getCell(1).font={bold:true,size:14,color:{argb:'FFFFFFFF'}};
  t.getCell(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};
  t.getCell(1).alignment={horizontal:'right',vertical:'middle'};
  ws.getRow(1).height=30;
  // Date row
  ws.mergeCells('A2:E2');
  const d=ws.addRow(['تاريخ الإنشاء: '+new Date().toLocaleDateString('ar-EG')]);
  d.getCell(1).font={size:10,color:{argb:'FF888888'}};
  d.getCell(1).alignment={horizontal:'right'};
  ws.getRow(2).height=18;
  // Empty row
  ws.addRow([]);
  // Header row
  ws.addRow(['المشروع','الوارد (ج)','المصاريف (ج)','الصافي (ج)','عدد القيود']);
  // Data rows
  rows.sort((a,b)=>b.net-a.net).forEach(r=>ws.addRow([r.name,_xlFmt(r.inc),_xlFmt(r.exp),_xlFmt(r.net),r.count]));
  // Totals row
  const tot=ws.addRow(['الإجمالي',
    rows.reduce((s,r)=>s+r.inc,0),
    rows.reduce((s,r)=>s+r.exp,0),
    rows.reduce((s,r)=>s+r.net,0),
    rows.reduce((s,r)=>s+r.count,0)
  ]);
  tot.eachCell(c=>{c.font={bold:true,size:11};c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F0E8'}};c.border={top:{style:'medium',color:{argb:'FF1D3C2A'}}};});
  ws.columns=[{width:28},{width:18},{width:18},{width:18},{width:14}];
  ...
  for(let i=5;i<ws.rowCount;i++){
    ws.getRow(i).eachCell(c=>{
      c.fill={type:'pattern',pattern:'solid',fgColor:{argb:i%2===0?'FFFAFAF8':'FFFFFFFF'}};
      c.font={size:10};c.alignment={horizontal:'right',vertical:'middle'};
