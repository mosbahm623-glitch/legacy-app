...
function runAdvFilter(){
  ...
  // تحويل yyyy-mm-dd لـ local date بدون timezone
  function toLocal(s){if(!s)return null;const[y,m,d]=s.split('-');return new Date(+y,+m-1,+d);}
  const from=fromStr?toLocal(fromStr):null;
  const to=toStr?(()=>{const d=toLocal(toStr);d.setHours(23,59,59);return d;})():null;
