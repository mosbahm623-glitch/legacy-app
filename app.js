...
async function bld(){
  ...
  const ct=Object.entries(cm).map(([n,rs])=>({n:safeSheet(n),r:rs.map(e=>[e.entry_no||'',e.description||'—',e.entry_date||'—',e.amount,e.contractor||'']),tr:6+rs.length}));
  ...
  return '<div id="sc-'+e.id+'" class="search-card-header">'+
    ...
    '<div style="padding:14px 16px">'+topInfo+
      ...
      '<div style="display:flex;gap:8px">'+
        ...
        '<button onclick="deleteSearchEntry(&quot;'+e.id+'&quot;,'+(e.entry_no||0))+')" class="search-edit-del-btn">🗑 حذف</button>'+
      '</div>'+
