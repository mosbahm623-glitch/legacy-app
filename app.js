...
function showScreen(s){
  ...
  curScreen=s;
  ['dash','daily','proj','adv','admin','rep','search','approvals','projStatus','timeline'].forEach(x=>{
  ...
  try{
    ...
    if(statusList){
      statusList.innerHTML=[
        {lbl:'ممتازة',val:excellent,color:'#6FCF97'},
        {lbl:'تحتاج متابعة',val:needFollow,color:'#C9A84C'},
        {lbl:'متعثرة',val:critical,color:'#EB5757'}
    ...
    if(donutCanvas&&window.Chart){
      ...
      donutCanvas._chartInst=new Chart(donutCanvas,{
        ...
        data:{
          ...
          datasets:[{data:[excellent||0,needFollow||0,critical||0],backgroundColor:['#6FCF97','#C9A84C','#EB5757'],borderWidth:0,hoverOffset:4}]
        },
        options:{
...
function _renderCashFlowChart(entries){
  ...
  cashCanvas._chartInst=new Chart(cashCanvas,{
    ...
    data:{
      labels:days,
      datasets:[
        {label:'الوارد',data:incData,borderColor:'#6FCF97',backgroundColor:'rgba(111,207,151,0.08)',tension:0.4,fill:true,pointRadius:2,borderWidth:2,pointHoverRadius:5},
...
function showImportPreview(ents,sk){
  ...
  table.innerHTML=`<thead><tr>${headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${
    ents.slice(0,100).map((e,i)=>{
      const proj=allProjects.find(p=>p.id===e.project_id);
      const typeCell=`<td><span style="padding:2px 8px;border-radius:8px;font-size:10px;font-weight:700;background:${e.type==='i'?'#EAF5EE':'#FFEBEE'};color:${e.type==='i'?'#1E6B3A':'#922B21'}">${e.type==='i'?'▲ وارد':'▼ مصروف'}</span></td>`;
...
async function addAdvEntry(){
  ...
  if(isNaN(amt)){notify('ادخل المبلغ','err');return;}
...
function updateDarkBtn(mode){
  ...
  const ico=document.getElementById('mbnThemeIco');
  const lbl=document.getElementById('mbnThemeLbl');
  if(ico)ico.textContent=mode==='day'?'☀️':'🌙';
  if(lbl)lbl.textContent=mode==='day'?'نهار':'ليل';
}
function applyUserTheme(){
  const key='lft_theme_'+(uid||'guest');
  const manual=localStorage.getItem(key+'_manual');
  let mode;
  if(manual){
    mode=localStorage.getItem(key)||'dark';
  }else{
    const h=new Date().getHours();
    mode=(h>=7&&h<19)?'day':'dark';
  }
