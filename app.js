...
async function loadDashboard(){
  try{
    ...
    heroEl.style.color=bal<0?'#C86060':'#7DBFA0';
    ...
    if(heroBar){
      ...
      heroBar.style.background=heroPct>90?'linear-gradient(90deg,#C86060,#C9A84C)':heroPct>70?'linear-gradient(90deg,#C9A84C,#7DBFA0)':'linear-gradient(90deg,#7DBFA0,#7BB8C8)';
    }
    ...
    const projColors=['#7DBFA0','#7BB8C8','#C9A84C','#9B7ACC','#C87858','#C87878','#7898C8','#C8A870'];
    ...
    container.innerHTML=sortedProjects.map((p,idx)=>{
      ...
      const fillColor=pct>90?'#C86060':pct>70?'#C9A84C':color;
...
function checkPwdStrength(v){
  ...
  const pct=score*20;
  const colors=['#C86060','#F39C12','#F39C12','#7DBFA0','#27AE60'];
  bar.style.width=pct+'%';
  bar.style.background=colors[Math.min(score-1,4)]||'#C86060';
}
...
async function downloadDashPDF(){
  ...
  thead tr{background:#1D3C2A}th{color:#E8E0D0;padding:9px 8px;text-align:right;font-size:10px}
...
function runAdvFilter(){
  ...
  ${filtered.length===0?'<div class="emp">لا يوجد مصروفات في هذه الفترة</div>':
    filtered.map(e=>`<div class="rw">
      ...
      <span style="color:#C86060;font-weight:700">▼ ${fn(e.amount)} ج</span>
    </div>`).join('')}`;
}
  ...
  try{
    ...
    <style>
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      th{color:#E8E0D0;padding:8px;text-align:right;font-size:10px;font-weight:700}
      ...
      .print-btn{display:block;margin:0 auto 20px;background:#1D3C2A;color:#E8E0D0;border:none;padding:10px 30px;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit}
...
function runCashFlow(){
  ...
  document.getElementById('repCashResult').innerHTML=`
    <div class="cf-kpi-row">
      ...
      <div class="cf-kpi"><div class="cf-kpi-lbl">صافي التدفق</div><div class="cf-kpi-val" style="color:${net>=0?'#7BB8C8':'#C86060'}">${net>=0?'+':''}${fn(net)} ج</div></div>
    </div>
      ...
      ${rows.map(r=>{
        ...
        return `<div class="cf-row">
          <div class="cf-row-hdr">
            ...
            <div class="cf-row-net" style="color:${rn>=0?'#7DBFA0':'#C86060'}">${rn>=0?'+':''}${fn(rn)} ج</div>
          </div>
...
function runSummary(){
  ...
  document.getElementById('repSummaryResult').innerHTML=`
    ...
    <div class="cf-kpi-row" style="margin-bottom:20px">
      ...
      <div class="cf-kpi"><div class="cf-kpi-lbl">الصافي</div><div class="cf-kpi-val" style="color:${totNet>=0?'#7BB8C8':'#C86060'}">${totNet>=0?'+':''}${fn(totNet)} ج</div></div>
    </div>
    ...
    ${rows.sort((a,b)=>b.net-a.net).map(r=>`
      <div class="sum-proj-card">
        ...
        <div class="sum-proj-row"><span class="sum-row-lbl">الصافي</span><span class="sum-row-val" style="color:${r.net>=0?'#7DBFA0':'#C86060'};font-size:15px">${r.net>=0?'+':''}${fn(r.net)} ج</span></div>
...
async function repExportPDF(){
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    thead tr{background:#1D3C2A}
    th{color:#E8E0D0;padding:9px 8px;text-align:right;font-size:10px;font-weight:700;letter-spacing:.3px}
    td{padding:7px 8px;border-bottom:1px solid #f0f0f0;vertical-align:middle}
    tr:nth-child(even) td{background:#fafaf8}
...
function repAdvExportPDF(){
  ...
  h1{color:#1D3C2A;font-size:20px;margin:0}.logo{font-size:24px;font-weight:900;color:#E8E0D0;background:#1D3C2A;padding:8px 16px;border-radius:8px}
  ...
  th{background:#1D3C2A;color:#E8E0D0;padding:9px;text-align:right}
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  th{background:#1D3C2A;color:#E8E0D0;padding:9px 10px;font-size:12px}
  td{padding:8px 10px;border-bottom:1px solid #eee}
  tr:nth-child(even) td{background:#fafafa}
  .total-row td{background:#1D3C2A;color:#E8E0D0;font-weight:700;font-size:13px}
...
function renderNpOnline(){
  ...
  body.innerHTML=sorted.map(u=>{
    ...
    const lastSeenTxt=u.is_online?'<span style="color:#7DBFA0;font-size:10px">● متصل الآن</span>':(u.last_seen?`آخر ظهور: ${notifFmtTime(u.last_seen)}`:'لم يتصل بعد');
