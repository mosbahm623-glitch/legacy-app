...
async function loadDashboard(){
  try{
    ...
    heroEl.style.color=bal<0?'#EB5757':'#6FCF97';
    ...
    if(heroBar){
      ...
      heroBar.style.background=heroPct>90?'linear-gradient(90deg,#EB5757,#F2C94C)':heroPct>70?'linear-gradient(90deg,#F2C94C,#6FCF97)':'linear-gradient(90deg,#6FCF97,#56CCF2)';
    }
    ...
    const projColors=['#6FCF97','#56CCF2','#F2C94C','#BB86FC','#F7845A','#FF8A80','#82B1FF','#FFD180'];
    ...
    container.innerHTML=sortedProjects.map((p,idx)=>{
      ...
      const fillColor=pct>90?'#EB5757':pct>70?'#F2C94C':color;
...
function re(){
  ...
  if(cTab==='m'){
    ...
    el.innerHTML=mqs.map((m,idx)=>{
      ...
      const kpis=hasTypes?`<div class="is5"><div class="kpi-inc"><div class="lbl-sm">💰 دفعات</div><div class="kpi-val-inc">${fn(m.pay)}</div></div><div class="kpi-work"><div class="lbl-sm">🔨 أعمال</div><div class="kpi-val-work">${fn(m.work)}</div></div><div class="kpi-mat"><div class="lbl-sm">🔩 مصنعيات</div><div class="kpi-val-mat">${fn(m.mat)}</div></div><div style="background:${rem>=0?'#F0FFF4':'#FFF0F0'};border-radius:8px;padding:8px;text-align:center"><div class="lbl-sm">${rem>=0?'الباقي معاه':'مستحق عليك'}</div><div style="font-weight:900;color:${rem>=0?'#1D3C2A':'#922B21'};font-size:13px">${fn(Math.abs(rem))}</div></div></div>`:`<div class="is6"><span style="color:#888;font-size:12px">إجمالي المسحوب</span><span style="font-weight:700;color:#1D3C2A">${fn(m.pay+m.work+m.mat+m.other)} ج</span></div>`;
  ...
  if(hasMqTypes){
    ...
    html+=Object.entries(mqMap).map(([name,d])=>{
      ...
      const rows=d.rows.map(e=>{
        ...
        return '<div class="is15"><span class="is16">'+et+'</span><span style="flex:1;color:#444">'+(e.description||'—')+'</span><span style="font-weight:700;color:#555">'+fn(e.amount)+' ج</span></div>';
      }).join('');
      return '<div class="is17"><div class="is18"><span class="is19">👷 '+name+'</span></div><div style="padding:12px 14px"><div class="is20"><div class="kpi-inc"><div class="lbl-sm">💰 دفعات</div><div class="kpi-val-inc">'+fn(d.pay)+'</div></div><div class="kpi-work"><div class="lbl-sm">🔨 أعمال</div><div class="kpi-val-work">'+fn(d.work)+'</div></div><div class="kpi-mat"><div class="lbl-sm">🔩 مصنعيات</div><div class="kpi-val-mat">'+fn(d.mat)+'</div></div></div><div class="is21"><span style="font-size:12px;color:#666">الباقي معاه</span><span class="is22">'+fn(rem)+' ج</span></div>'+rows+'</div></div>';
    }).join('');
    html+='</div>';
  }
  el.innerHTML=html;
...
async function loadAdvList(){
  ...
  try{
    ...
    al.innerHTML=advances.map(a=>{
      ...
      return '<div class="adv-card" onclick="openAdv(\''+a.id+'\')"><div class="adv-card-h"><div class="adv-name">👤 '+a.person_name+ownerBadge+'</div><span class="adv-status '+(a.status==='open'?'open':'closed')+'">'+(a.status==='open'?'⏳ مفتوحة':'✅ مغلقة')+'</span></div>'+(a.notes?'<div class="is24">'+a.notes+'</div>':'')+'<div class="adv-nums"><div class="adv-num"><div class="adv-num-l">العهدة</div><div class="adv-num-v" style="color:#185FA5">'+fn(totalGiven)+'</div></div><div class="adv-num"><div class="adv-num-l">صرف</div><div class="adv-num-v" style="color:#922B21">'+fn(spent)+'</div></div><div class="adv-num"><div class="adv-num-l">الباقي</div><div class="adv-num-v" style="color:'+remColor+'">'+fn(rem)+'</div></div></div><div class="is25"><div class="is26"></div></div></div>';
    }).join('');
...
async function downloadDashPDF(){
  ...
  <div class="kpis">
    <div class="kpi is32"><div class="kpi-lbl">إجمالي الوارد</div><div class="kpi-val" style="color:#1E6B3A">▲ ${fn(d.inc)} ج</div></div>
    <div class="kpi box-err"><div class="kpi-lbl">إجمالي المصروف</div><div class="kpi-val" style="color:#922B21">▼ ${fn(d.exp)} ج</div></div>
  ...
  try{
    ...
    <div class="page">
      ...
      <table>
        ...
        <tbody>${rows}</tbody>
        <tfoot><tr style="background:#1D3C2A"><td colspan="5" class="is39">الإجمالي</td><td class="is40">${Number(totalSpent).toLocaleString('en-US')} ج</td></tr></tfoot>
      </table>
      ...
      <table>
        ...
        <tfoot><tr style="background:#185FA5"><td colspan="2" class="is41">الإجمالي</td><td class="is42">${Number(totalGiven).toLocaleString('en-US')} ج</td></tr></tfoot>
      </table>`:''}
      <div class="footer">
  ...
  _loadChartJs(()=>{
    ...
    ctx._chartInst=new Chart(ctx,{
      ...
      options:{
        ...
        scales:{x:{ticks:{color:'#888',font:{size:11},maxRotation:45},grid:{display:false}},y:{ticks:{color:'#888',font:{size:10},callback:v=>fn(v)},grid:{color:'rgba(255,255,255,.06)'}}}
      }
    });
  });
}
  ...
  _loadChartJs(()=>{
    ...
    ctx._chartInst=new Chart(ctx,{
      ...
      options:{
        responsive:true,maintainAspectRatio:false,
        plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`${ctx.dataset.label}: ${fn(ctx.parsed.y)} ج`}}},
        scales:{x:{ticks:{color:'#888',font:{size:11},maxRotation:30},grid:{display:false}},y:{ticks:{color:'#888',font:{size:10},callback:v=>fn(v)},grid:{color:'rgba(255,255,255,.06)'}}}
      }
    });
  });
}
...
function cashExportPDF(){
  const el=document.getElementById('repCashResult');
  if(!el||!el.innerHTML.trim()||el.innerHTML.includes('لا توجد')){alert('اعرض التقرير أولاً');return;}
  ...
  const chartImg=canvas?`<img src="${canvas.toDataURL('image/png')}" style="width:100%;max-height:220px;object-fit:contain;margin-bottom:20px;border-radius:8px">` :'';

  const html=`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>التدفق النقدي</title>
  <style>*{font-family:Arial,sans-serif}body{padding:24px;direction:rtl}
  .hdr{border-bottom:3px solid #1D3C2A;padding-bottom:12px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center}
  h1{color:#1D3C2A;font-size:20px;margin:0}h2{font-size:13px;color:#666;margin:4px 0 0}
  table{width:100%;border-collapse:collapse;font-size:13px}
  th{background:#1D3C2A;color:#D4C49A;padding:9px;text-align:right}
  td{padding:8px 9px;border-bottom:1px solid #eee}
  .pos{color:#1E6B3A;font-weight:700}.neg{color:#922B21;font-weight:700}
  tfoot td{background:#f5f5f5;font-weight:700;border-top:2px solid #1D3C2A}
  @media print{button{display:none}}</style></head><body>
  <div class="hdr"><div><h1>💰 تقرير التدفق النقدي</h1><h2>Legacy Fine Touch · ${new Date().toLocaleDateString('ar-EG')}</h2></div></div>
  ${chartImg}
  <table><thead><tr><th>الشهر</th><th>الوارد</th><th>المصاريف</th><th>الصافي</th></tr></thead>
  <tbody>${rows.map(r=>{const n=r.inc-r.exp;return`<tr><td>${_monthLabel(r.y,r.m)}</td><td class="pos">+${fn(r.inc)} ج</td><td class="neg">-${fn(r.exp)} ج</td><td class="${n>=0?'pos':'neg'}">${n>=0?'+':''}${fn(n)} ج</td></tr>`;}).join('')}</tbody>
  <tfoot><tr><td>الإجمالي</td><td class="pos">+${fn(rows.reduce((s,r)=>s+r.inc,0))} ج</td><td class="neg">-${fn(rows.reduce((s,r)=>s+r.exp,0))} ج</td><td class="${rows.reduce((s,r)=>s+r.inc-r.exp,0)>=0?'pos':'neg'}">${rows.reduce((s,r)=>s+r.inc-r.exp,0)>=0?'+':''}${fn(Math.abs(rows.reduce((s,r)=>s+r.inc-r.exp,0)))} ج</td></tr></tfoot>
...
async function cashExportExcel(){
  ...
  ws.addRow(['الشهر','الوارد','المصاريف','الصافي']);
  ws.getRow(1).eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFD4C49A'},bold:true};c.alignment={horizontal:'right'};});
  rows.forEach(r=>{const n=r.inc-r.exp;ws.addRow([_monthLabel(r.y,r.m),r.inc,r.exp,n]);});
  const tot=ws.addRow(['الإجمالي',rows.reduce((s,r)=>s+r.inc,0),rows.reduce((s,r)=>s+r.exp,0),rows.reduce((s,r)=>s+r.inc-r.exp,0)]);
  tot.eachCell(c=>{c.font={bold:true};c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F5F5'}};});
...
function summaryExportPDF(){
  ...
  const html=`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>الملخص الدوري</title>
  <style>*{font-family:Arial,sans-serif}body{padding:24px;direction:rtl}
  .hdr{border-bottom:3px solid #1D3C2A;padding-bottom:12px;margin-bottom:20px;display:flex;justify-content:space-between}
  h1{color:#1D3C2A;font-size:20px;margin:0}h2{font-size:13px;color:#666;margin:4px 0 0}
  table{width:100%;border-collapse:collapse;font-size:13px}
  th{background:#1D3C2A;color:#D4C49A;padding:9px;text-align:right}
  td{padding:8px 9px;border-bottom:1px solid #eee}
  ...
  tfoot td{background:#f5f5f5;font-weight:700;border-top:2px solid #1D3C2A}
  @media print{button{display:none}}</style></head><body>
  <div class="hdr"><div><h1>📋 الملخص الدوري</h1><h2>Legacy Fine Touch · ${new Date().toLocaleDateString('ar-EG')}</h2></div></div>
  ${chartImg}
  <table><thead><tr><th>المشروع</th><th>الوارد</th><th>المصاريف</th><th>الصافي</th><th>القيود</th></tr></thead>
  <tbody>${rows.map(r=>`<tr><td>${r.name}</td><td class="pos">+${fn(r.inc)} ج</td><td class="neg">-${fn(r.exp)} ج</td><td class="${r.net>=0?'pos':'neg'}">${r.net>=0?'+':''}${fn(r.net)} ج</td><td style="text-align:center">${r.count}</td></tr>`).join('')}</tbody>
  <tfoot><tr><td>الإجمالي</td><td class="pos">+${fn(rows.reduce((s,r)=>s+r.inc,0))} ج</td><td class="neg">-${fn(rows.reduce((s,r)=>s+r.exp,0))} ج</td><td class="${rows.reduce((s,r)=>s+r.net,0)>=0?'pos':'neg'}">${rows.reduce((s,r)=>s+r.net,0)>=0?'+':''}${fn(Math.abs(rows.reduce((s,r)=>s+r.net,0)))} ج</td><td style="text-align:center">${rows.reduce((s,r)=>s+r.count,0)}</td></tr></tfoot>
  </table><script>window.onload=()=>window.print();<\/script></body></html>`;
  const w=window.open('','_blank');w.document.write(html);w.document.close();
}
...
async function summaryExportExcel(){
  ...
  ws.addRow(['المشروع','الوارد','المصاريف','الصافي','عدد القيود']);
  ws.getRow(1).eachCell(c=>{c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1D3C2A'}};c.font={color:{argb:'FFD4C49A'},bold:true};c.alignment={horizontal:'right'};});
  rows.forEach(r=>ws.addRow([r.name,r.inc,r.exp,r.net,r.count]));
  ws.columns=[{width:25},{width:18},{width:18},{width:18},{width:12}];
  const buf=await wb.xlsx.writeBuffer();
  const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([buf],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}));
...
function duesExportPDF(){
  ...
  const rows=window._duesRows.map((r,i)=>`<tr style="background:${i%2===0?'#fff':'#fafafa'}"><td>${r.proj}</td><td>👷 ${r.mq}</td><td style="text-align:center">${fn(r.work+r.mat)} ج</td><td style="text-align:center;color:#1E6B3A">${fn(r.pay)} ج</td><td class="is60">${fn(r.due)} ج</td></tr>`).join('');
  ...
  .footer{margin-top:20px;font-size:10px;color:#bbb;text-align:center;border-top:1px solid #eee;padding-top:12px}
  @media print{button{display:none}}</style></head><body>
  <div class="header"><div><h1>⚠️ تقرير مستحقات المقاولين</h1><h2 class="is61">تم الإنشاء: ${new Date().toLocaleDateString('ar-EG')}</h2></div><img src="${LOGO_B64}" style="height:55px;object-fit:contain"></div>
  <table><thead><tr><th>المشروع</th><th>المقاول</th><th>أعمال + مصنعيات</th><th>مدفوع</th><th>المستحق</th></tr></thead>
  <tbody>${rows}</tbody>
  <tfoot><tr><td colspan="4" style="text-align:right">إجمالي المستحقات</td><td class="is62">${fn(window._duesTotal)} ج</td></tr></tfoot></table>
  ...
  try{
    ...
    if(hasAdv){
      ...
      html+=advRows.map(r=>{
        ...
        const detail=isAdv
          ?`<span class="title-sm">${r.person_name||'—'}</span>${r.notes?` <span style="color:#888;font-size:12px">· ${r.notes}</span>`:''}`
          :`<span class="title-sm">${fn(r.amount)} ج</span> <span style="color:#888;font-size:12px">لـ ${advMap[r.advance_id]||'—'}</span> <span style="color:#aaa;font-size:11px">· ${r.inst_note||'دفعة'}</span>`;
        return `<div class="is94">
          <div class="is95">
