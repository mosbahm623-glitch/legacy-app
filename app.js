...
(function(){
  ...
  function createPicker(){
    ...
    const _dpApplyTheme=()=>{
      ...
      d.style.background=dark?'#1e2a1e':'#ffffff';
      d.style.borderColor=dark?'rgba(212,196,154,.3)':'#d0d0d0';
      d.style.color=dark?'#e8e0d0':'#1a1a1a';
      const label=d.querySelector('#dp-label');
      if(label)label.style.color=dark?'#e8e0d0':'#1a1a1a';
      const prev=d.querySelector('#dp-prev');
      if(prev)prev.style.color=dark?'rgba(212,196,154,.7)':'#555';
      const next=d.querySelector('#dp-next');
      if(next)next.style.color=dark?'rgba(212,196,154,.7)':'#555';
      d.querySelectorAll('#dp-hdr div').forEach(c=>{c.style.color=dark?'rgba(212,196,154,.5)':'#888';});
    };
    _dpApplyTheme();
  ...
  function renderDP(){
    ...
    for(let d2=1;d2<=days;d2++){
      ...
      const isSel=selD===d2;
      btn.textContent=d2;
      const _nc=_dark?'#e8e0d0':'#1a1a1a';
      const _nb=_dark?'transparent':'none';
      const _tb=_dark?'rgba(29,60,42,.6)':'#F5EFD7';
      const _tc=_dark?'#C9A84C':'#1D3C2A';
      const _hb=_dark?'rgba(212,196,154,.1)':'#f0f4f0';
      btn.style.cssText='border:none;cursor:pointer;border-radius:7px;padding:6px 2px;font-size:12px;font-family:inherit;width:100%;transition:background .1s;'+(isSel?'background:#1D3C2A;color:#fff;font-weight:700;':(isToday?`background:${_tb};color:${_tc};font-weight:700;`:`background:${_nb};color:${_nc};`));
...
function re(){
  ...
  if(cTab==='j'){const j=gJ();if(!j.length){el.innerHTML='<div class="emp">لا توجد قيود بعد</div>';return;}
    ...
    el.innerHTML=pager+slice.map(e=>{const ii=e.type==='i';const mq=e.contractor?'<span class="qb">'+e.contractor+'</span>':'';const ab=e.advance_id?'<span class="ab-badge">عهدة</span>':'';const no='<span class="nb">#'+(e.seq||e.entry_no||'?')+'</span>';const del=canEdit?'<button class="db" onclick="event.stopPropagation();de(\''+e.id+'\')">🗑</button>':'';return '<div class="rw'+(canEdit?' clk':'')+'" data-eid="'+e.id+'" onclick="oe(\''+e.id+'\')"><div class="ri"><div class="rd">'+ab+mq+(e.description||'—')+no+'</div><div class="rm">'+cleanDate(e.entry_date)+' · '+(ii?'وارد':e.category||'—')+'</div></div><div class="flex-center-gap"><div style="text-align:left"><div class="ra '+(ii?'pos':'neg')+'">'+(ii?'+':'-')+fn(Math.abs(e.amount))+'</div><div class="jb '+(e.bal<0?'neg':e.bal>0?'pos':'')+'">'+fn(e.bal)+' ج</div></div>'+del+'</div></div>';}).join('')+pager;return;}
  if(cTab==='m'){
    const mqMap={};
  ...
  try{
    ...
    if(advEntries.length===0&&pendingAdvEntries.length===0){
      ...
      const approvedHtml=advEntries.map(e2=>{
        ...
        var db2=canEditAdv?`<button class='db' onclick='editAdvEntry("${e2.id}")' style='color:#1D3C2A'>✏️</button><button class='db' onclick='delAdvEntry("${e2.id}")'>🗑</button>`:'';
...
function runRepFilter(){
  ...
  document.getElementById('repResult').innerHTML=`
    <div class="cf-kpi-row">
      ...
      <div class="cf-kpi"><div class="cf-kpi-lbl">الرصيد</div><div class="cf-kpi-val" style="color:${bal>=0?'#7BB8C8':'#C86060'}">${bal>=0?'+':''}${fn(bal)} ج</div></div>
    </div>
    ...
    <div class="cf-bars">
      ${bRows.map(r=>{
        ...
        const rn=r.inc-r.exp;
        return `<div class="cf-row">
          <div class="cf-row-hdr"><div class="cf-row-lbl">${_monthLabel(r.y,r.m)}</div><div class="cf-row-net" style="color:${rn>=0?'#7DBFA0':'#C86060'}">${rn>=0?'+':''}${fn(rn)} ج</div></div>
          <div class="cf-bar-wrap">
            <div class="cf-bar-row"><div class="cf-bar-lbl">وارد</div><div class="cf-bar-track"><div class="cf-bar-fill inc" style="width:${iw}%"></div></div><div class="cf-bar-amt inc">+${fn(r.inc)} ج</div></div>
            <div class="cf-bar-row"><div class="cf-bar-lbl">مصروف</div><div class="cf-bar-track"><div class="cf-bar-fill exp" style="width:${ew}%"></div></div><div class="cf-bar-amt exp">-${fn(r.exp)} ج</div></div>
          </div></div>`;
