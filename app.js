...
function showScreen(s){
  ...
  curScreen=s;
  ['dash','daily','proj','projList','archiveScreen','adv','admin','rep','search','approvals','projStatus','timeline'].forEach(x=>{
    const el=document.getElementById(x+'Screen');
    if(el)el.style.display=x===s?'block':'none';
  });
  if(s==='projList'){buildProjListScreen();}
  if(s==='archiveScreen'){loadArchivedProjects();}
...
function goToProject(pid){
  showScreen('proj');
  // أضيف زرار الرجوع لقايمة المشاريع
...
async function loadAllProjects(){
  ...
  [allProjects,allEntries]=await Promise.all([
    sb('projects?is_archived=eq.false&order=created_at'),
...
async function archiveProject(){
  const p=curP();
  if(!p)return;
  if(!confirm(`أرشفة مشروع "${p.name}"؟\nالمشروع هيتخفى من القايمة بس بياناته محفوظة.`))return;
  try{
    await sb('projects?id=eq.'+curPid,'PATCH',{is_archived:true});
    // filter محلي — بدون reload
    allProjects=allProjects.filter(pr=>pr.id!==curPid);
    projects=projects.filter(pr=>pr.id!==curPid);
    notify(`📦 تم أرشفة مشروع: ${p.name}`,'ok');
    curPid=projects.length?projects[0].id:null;
    buildProjListScreen();
    showScreen('projList');
...
async function loadArchivedProjects(){
  try{
    const archived=await sb('projects?is_archived=eq.true&order=created_at');
    const grid=document.getElementById('archivedCardsGrid');
    if(!grid)return;
    if(!archived.length){grid.innerHTML='<div class="emp">لا توجد مشاريع مؤرشفة</div>';return;}
    grid.innerHTML=archived.map(p=>`
      <div class="proj-card archived-card">
        <div class="proj-card-name">📦 ${p.name}</div>
        <div class="proj-card-row"><span class="proj-card-lbl">تاريخ البداية</span><span class="proj-card-val">${p.start_date||'—'}</span></div>
        <div class="proj-card-row"><span class="proj-card-lbl">تاريخ الإغلاق</span><span class="proj-card-val">${p.close_date||'—'}</span></div>
        <div class="proj-card-divider"></div>
        <button class="btn-restore" onclick="restoreProject('${p.id}','${p.name.replace(/'/g,"\\'")}')">↩ استرجاع</button>
      </div>`).join('');
  }catch(e){notify('❌ خطأ: '+e.message,'err');}
}
async function restoreProject(pid,name){
  if(!confirm(`استرجاع مشروع "${name}"؟`))return;
  try{
    await sb('projects?id=eq.'+pid,'PATCH',{is_archived:false});
    notify(`✅ تم استرجاع مشروع: ${name}`,'ok');
    await loadAllProjects();
    await loadProjects();
    buildProjListScreen();
...
function rp(){
  ...
  if(!projects.length){showScreen('projList');return;}
