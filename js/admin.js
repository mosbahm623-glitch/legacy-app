...
async function bldClient(){
  ...
  // الوارد
  const wI=wb.addWorksheet('الوارد',{tabColor:{argb:'FF2A5C38'}});[8,16,32,16,20].forEach((w,i)=>wI.getColumn(i+1).width=w);bs(wI,'حركة الوارد  —  '+p.name,5);wI.getRow(5).height=22;['#','رقم القيد','البيان','التاريخ','المبلغ (ج)'].forEach((v,i)=>{const c=wI.getCell(5,i+1);c.value=v;F(c,G2);T(c,'FFFFFF',10,true);A(c,'center');});let ir=6;ic.forEach((row,ix)=>{wI.getRow(ir).height=21;const cs=[wI.getCell('A'+ir),wI.getCell('B'+ir),wI.getCell('C'+ir),wI.getCell('D'+ir),wI.getCell('E'+ir)];cs.forEach(c=>{BD(c);if(ix%2===0)F(c,G6);});cs[0].value=ix+1;A(cs[0],'center');T(cs[0],G1,9,true);cs[1].value=row[0];A(cs[1],'center');T(cs[1],G1,9,true);cs[2].value=row[1];A(cs[2],'right');T(cs[2],'1A1A1A',10);cs[3].value=row[2];A(cs[3],'center');T(cs[3],'666666',9);cs[4].value=row[3];A(cs[4],'left');N(cs[4]);T(cs[4],BL,10,true);ir++;});wI.getRow(ir).height=28;MC(wI,'A'+ir,'D'+ir);wI.getCell('A'+ir).value='الإجمالي';F(wI.getCell('A'+ir),G1);T(wI.getCell('A'+ir),B1,11,true);A(wI.getCell('A'+ir),'right');const iD=wI.getCell('E'+ir);iD.value={formula:ic.length?'SUM(E6:E'+(ir-1)+')':'0',result:inc};F(iD,G1);T(iD,B1,11,true);A(iD,'left');N(iD);af(wI,ir+2,'Legacy Fine Touch  |  الوارد  |  '+p.name,5);
...
function initRealtime(){
  ...
  try{
    ...
    _rtAdvCh=_sbc.channel('advances-all')
      ...
      .subscribe();
  }catch(e){console.warn('realtime init:',e);setSav('⚠️ الـ realtime غير متاح','ng');} // صامت متعمد
}
