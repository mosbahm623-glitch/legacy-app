...
${allProjectsMap[e.project_id]?.client_phone&&!isExp?`<div style="margin:0 24px 16px;text-align:center"><a href="https://web.whatsapp.com/send?phone=${allProjectsMap[e.project_id].client_phone}&text=${encodeURIComponent('مرحباً، نفيدكم باستلام مبلغ '+fn2(Math.abs(e.amount))+' ج'+String.fromCharCode(10)+'المشروع: '+proj+String.fromCharCode(10)+'رقم الإيصال: '+(e.seq||''))}" style="display:inline-flex;align-items:center;gap:8px;background:#25D366;color:#fff;padding:10px 24px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500">إرسال واتساب للعميل</a></div>`:''}
